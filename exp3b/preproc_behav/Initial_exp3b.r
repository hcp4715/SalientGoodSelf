...
Mplots <- function(saveDir = traDir, curDir = curDir, expName = 'exp7', dData,rtData){
      ...
      if ('Identity' %in% colnames(dData)){
          ...
          ggsave(fileName, P.dprime, scale = 1,height = 6, width = 8, dpi = 300, family = "Times",path = saveDir)
          # save the RT plot
          fileName = paste0('p_',expName,'_match_RT','.pdf')
          ggsave(fileName, P.rt, scale = 1,height = 6, width = 8, dpi = 300, family = "Times",path = saveDir)
          # save the both      
          fileName = paste0('p_',expName,'_match_','.tiff')
          setwd(saveDir)
          tiff(fileName, height = 12, width = 16,  units = 'in', res = 300)
