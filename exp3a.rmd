...
# criteria
exp3a_sdt_p <- fun_plot_sdt_val(exp3a_sdt_m1)
```
...
hypothesis(exp3a_rt_m1, "ismatch < 0")  # Effect of matchness: Match < mis-match, p = 1
hypothesis(exp3a_rt_m1, "ValenceGood:ismatch < 0")  # Match good < Match Neutral, p = 0.99
hypothesis(exp3a_rt_m1, "ValenceBad:ismatch > 0")   # Match Bad > Match Neutral, p = 0.99
hypothesis(exp3a_rt_m1, "(ValenceGood:ismatch - ValenceBad:ismatch) < 0")   # Match Good < Match Bad, p = 1

exp3a_rt_p <- fun_plot_rt_val(exp3a_rt_m1)
```

```{r plot-exp3a-BGLM, fig.cap="Exp3a: Results of Bayesian GLM analysis.",  fig.height=4.5, fig.width=9, warning=FALSE}
library(patchwork)
exp3a_sdt_p[[1]] + exp3a_sdt_p[[2]] + exp3a_sdt_p[[3]] + exp3a_sdt_p[[4]] + exp3a_rt_p[[1]] + exp3a_rt_p[[2]] + exp3a_rt_p[[3]] + exp3a_rt_p[[4]] + plot_annotation(tag_levels = 'A')  + plot_layout(nrow = 4, byrow = FALSE)
```
In reaction times, we found that same trends in the match trials: while the shapes associated with good self if greater than with good other, the direction is reversed for neutral and negative condition.
