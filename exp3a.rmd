...
exp3a_sdt_p <- exp3a_sdt_m1 %>%
  ...
  #geom_line(alpha = .01) +
  tidybayes::stat_halfeye(aes(fill = Identity), alpha = 0.6) + # position=position_dodge(width = 0.1)
  ...
  #facet_grid(~ ismatch) +
  labs(y=expression('Posteior distribution of '~italic(d)~' prime')) +  
  theme_classic()
```
...
exp3a_rt_p <- exp3a_rt_m1 %>%
  ...
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  labs(y=expression('Posteior distribution of RTs')) +  
  # facet_grid(~ ismatch) +
  theme_classic()
...
# below is for grant writing
p <- exp3a_rt_p + exp3a_sdt_p + plot_annotation(tag_levels = 'A')   + plot_layout(guides = "collect") 
ggsave('exp3a_plot_posterior.png', p, width = 9, height = 4.5)
```
