...
However, the maximum likelihood estimate of parameters of EVSDT may ignored the uncertainty in estimates of the parameters. Here we adopted a Bayesian generalized linear model, implemented by (Bürkner 2017; Stan Development Team 2016) to model the data. In the GLM model, we assume that the outcome of each trial is Bernoulli distributed (binomial with 1 trial), with probability $p_{ij}$ that $y_{ij} = 1$. 
...
The nonlinear model
$$p_{ij} = \Phi(d'_{j}IsMatch_{ij} - c) $$
This nonlinear model gives us direct estimate of $c$ and $d'$. 
