...
For the reaction time, there are many criticism about using the mean RTs as the representation of each participant [@Rousselet_2019], to better catpure a representative parameter for RTs, we used log normal distribution (https://lindeloev.github.io/shiny-rt/#34_(shifted)_log-normal). This distribution has three parameters: $\mu$, and $\sigma$. $\mu$ is the mean of the logNormal distribution, and $\sigma$ is the disperse of the distribution, while shift is the earliest possible response.
...
# have a loot at a few participants' data
set.seed(123)
...
# Mismatch:Neutral - Intercept
# Mismatch:Bad     - Intercept  + ValenceBad
# Mismatch:Good    - Intercept  + ValenceGood
# Match: Neutral   - Intercept  + ismatch_num
# Match: Bad       - Intercept  + ismatch_num + ValenceBad+ ValenceBad:ismatch_num 
# Match: Good      - Intercept  + ismatch_num + ValenceGood+ ValenceGood:ismatch_num 
...
library(qgraph)
qgraph(df.dist.mat_t, layout='spring', vsize=3)
