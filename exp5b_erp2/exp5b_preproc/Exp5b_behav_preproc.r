...
df5b_d1.acc_w <- dcast(df5b_d1.acc, Subject ~  Match + Identity + Morality,value.var = "ACC")
...
df5b_d1.V.RT.subj <- summarySEwithin(df5b_d1.V.RT,measurevar = 'RT', withinvar = c('Subject','Match',"Identity",'Morality'), idvar = 'Subject',na.rm = TRUE)

## long to wide
df5b_d1.V.RT.subj_w <- dcast(df5b_d1.V.RT.subj, Subject ~ Match + Identity + Morality ,value.var = "RT") 
...
df5b_d1.v.sum_rt_acc_l <- merge(df5b_d1.acc,df5b_d1.V.RT.subj,by = c("Subject","Match","Identity","Morality"))
...
Mplots(saveDir = resDir, curDir = curDir, expName = 'exp5a', df5b_d1.V.dprime_l,rtdata)
