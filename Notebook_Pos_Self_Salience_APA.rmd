...
library(patchwork)
# (p_rt1 | p_dprime1)
p_rt1 + p_dprime1 + plot_annotation(tag_levels = 'A')  + plot_layout(nrow = 1, byrow = FALSE)
```
...
In this part, we report three experiments (3a, 3b, 4a, 4b, and 6b) that aimed at testing whether the moral valence effect found in the previous experiments can be modulated by the self-referential processing. These analysis included 108 participants.
...
# 
# Note: initalization failed for a few times for full model. need to re-consider the model
sdt_ms_m1 <- df_ms %>%
...
#### test the code in tidybayes: https://github.com/mjskay/tidybayes/blob/master/R/spread_draws.R#L324
...
#### end of the test code, thes snapet of code was used for an issue on git: https://github.com/mjskay/tidybayes/issues/282

# plot the population level parameter (d prime)
sdt_ms_m1_p <- sdt_ms_m1 %>%
  # get the traces of population level parameters
  tidybayes::gather_draws(`b_IdentitySelf:ValenceNeutral`, `b_IdentityOther:ValenceNeutral`,
                          `b_IdentitySelf:ValenceBad`    , `b_IdentityOther:ValenceBad`,
                          `b_IdentitySelf:ValenceGood`   , `b_IdentityOther:ValenceGood`,
                          `b_IdentitySelf:ValenceNeutral:ismatch`, `b_IdentityOther:ValenceNeutral:ismatch`,
                          `b_IdentitySelf:ValenceBad:ismatch`, `b_IdentityOther:ValenceBad:ismatch`,
                          `b_IdentitySelf:ValenceGood:ismatch`, `b_IdentityOther:ValenceGood:ismatch`) %>%
  # create two columns for two independent factors.
