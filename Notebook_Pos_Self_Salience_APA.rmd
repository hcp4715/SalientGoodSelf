...
affiliation:
  - id            : "1"
    institution   : "School of Psychology, Nanjing Normal University, 210024 Nanjing, China"
  - id            : "2"
...
Given the importance of moral character, to successfully navigate in a social world, a person needs to both evaluate others' moral character and behave in a way that she/he is perceived as a moral person, or at least not a morally bad person. Maintaining a moral self-view is as important as making judgments about others' moral character [@ellemers_psychology_2019]. Indeed, previous studies found that people maintain a positive moral self-view even after dishonest behavior [@monin_dynamic_2009] and that people evaluate themselves as morally superior to others [@strohminger_true_2017; @klein_maybe_2016; @tappin_illusion_2017]. Recent theorists further integrated the moral judgment and moral self-view, proposed a person-centered account for moral psychology, which focused on the individuals in moral evaluation instead of acts [@uhlmann_person-centered_2015]. Under this framework, previous seemingly contradicting phenomenons can be explained. For example, whether people decide to whistleblow depends on how their relationship of the target [e.g., @waytz_whistleblowers_2013]. 
...
# 87432.5 = 24.3 hours
std_val_m1 <- brms::brm(saymatch ~ 0 + Valence + Valence:ismatch + 
...
plot(hypothesis(std_val_m1,
                "ValenceBad:ismatch > ValenceNeutral:ismatch"))

plot(hypothesis(std_val_m1,
                "ValenceGood:ismatch > ValenceNeutral:ismatch"))

# std_val_m1_p <- std_val_m1 %>%
...
std_val_m1_p <- std_val_m1 %>%
  ...
  dplyr::mutate(params = factor(params, levels = c('d prime', 'criterion')),
...
# check the nrows in each condition
df_m1_plot_sdt %>% 
...
df_m1_plot_sdt %>%
  ...
  # dplyr::filter(str_detect(term, 'dprime')) %>%
  ggplot2::ggplot(aes(y = Experiments, x = value)) +
  # tidybayes::geom_halfeyeh(color = Valence)
...
df_m1_plot_sdt %>%
  ...
  dplyr::mutate(Experiments = fct_rev(Experiments)) %>%
  ggplot(aes(y = Experiments)) +
  ggridges::geom_density_ridges(
    aes(x = value, fill = paste(Experiments, Valence)), 
    alpha = .7, color = "gray") + # , from = 1, to = 4
  labs(
    x = "d prime",
    y = "Experiments",
    title = "D prime across experiments",
    subtitle = "Overall is the populational level",
    caption = "Salient Moral Character"
  ) +
  #scale_y_discrete(expand = c(0, 0)) +
  #scale_x_continuous(expand = c(0, 0)) +
  scale_fill_cyclical(
    breaks = c("Exp1a_THU good", "Exp1a_THU neutral", "Exp1a_THU bad"),
    labels = c(`Exp1a_THU good` = "Good", `Exp1a_THU neutral` = "Neutral", `Exp1a_THU bad` = "Bad"),
    values = c("#ff8080", "#8080ff", "#90EE90"), # "#ff0000", "#0000ff", "#008000", 
    name = "Valence", guide = "legend"
  ) #+
...
# plot the distribution of 10 randomaly selected paraticipants
df_moral %>%
  dplyr::mutate(RT_sec = RT/1000) %>%  # log RT in seconds
...
# Total execution time: 124845.8 seconds = 34.7 hours
# RT_val_m1 <- df_moral %>%
#   dplyr::mutate(RT_sec = RT/1000) %>% # log RT in seconds
#   dplyr::filter(ACC == 1) %>%         # only correct trials
