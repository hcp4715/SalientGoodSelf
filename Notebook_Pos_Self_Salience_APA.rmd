...
The unidentified good person is me: Spontaneous self-referential processing for good moral character
...
People experience a substantial amount of moral events in everyday life [e.g., @hofmann_morality_2014]. Whether we are the agent, target, or the third party of a moral event, we are all judging the moral character of people involved in as "good" or "bad" or judging the behaviors as "right" or "wrong". In the later case, we are implicitly judging the moral character of the agent as "good" vs. "bad" [@uhlmann_person-centered_2015]. Moral character is so important in social life that most of people's conversation (gossips) are related to moral character of other (or, reputation) []. Also, evidence from studies of person perception and social evaluation revealed that morality is a basic dimension for social evaluation and it is weighed more than traits from other dimensions [@goodwin_moral_2014; @goodwin_moral_2015; @abele_navigating_2020; @willis_first_2006]. Also, the importance of moral character had been internalized to individuals' self-concept and the positive moral self is the most important aspect of identity [@strohminger_true_2017]. 

[No real perceptual studies on moral character] 
Given the importance of moral character, it is not surprised that people often claim that moral character related information are processed more efficiently in human information processing system, especially 'bad' agents (e.g., the introduction part of Siegel,et al. 2018 nhb). A scrutiny of the evidence, however, revealed few direct evidence. For example, while Schupp et al (2004), Ohman et al (2001) are cited as the evidence,  they are using facial expressions as stimuli, thus did not contain any moral meaning; Skowronski et al (1989), Fiske et al (1980), and Baumeister et al (2011) are often cited but they were not referring to moral character in specific but using negative social traits, which include competence and sociability as well. For instance, Pratto et al (1991) focused on the desirability of personal traits, which is not specific to moral character either. While Vanneste et al. (2007) tried to study the attentional grabbing effect of the facial expression when the agent decide not to cooperate, the explanation of the effect can not be specifically attribute to uncooperativeness but the facial expression itself, after all the participant who performed the dot-detection task have no idea about the moral character or behavior of that facial expression. In short, though researchers in the field have the intuition that that moral character, especially the bad one, is prioritized in information processing , direct evidence is scarce. The situation is like person perception, as @freeman_dynamic_2011 put it, studies in the person perception didn't try to explain the perceptual process, rather, they are trying to explain the higher-order social cognitive processes that come after. That is, these studies are perception of moral character without perceptual process. Therefore, it remains unclear (1) whether moral character related information are prioritized in information processing (e.g., perception), and (2) the underlying mechanism of the prioritization effect of moral character related information.

[Challenge: operationalization of moral character in laboratory settings] 
The scarcity of studies on low-level information process of moral character is not without reasons. When trying to study moral character's effect on information process (e.g., perception), one big challenge lies in the difficulty to operationalize the moral character. Morality is defined by context. Whether a behavior should be judged as immoral depends on a number of factors such as intention, consequences [cushman, young]. Also, whether a behavior is morality relevant depends on cultural and social norm [fiske, haidt]. These factors, when studied in laboratory settings, have to be carefully controlled and manipulated using complex verbal scenarios. However, when in the classic cognitive paradigms, long sentences or complicated scenarios are prohibited because it's hard to dissociated different processes then. To solve this issue, two approaches emerged in the last decade. The first approach used direct learning. For example, @shore_social_2013 asked participants first interact with a person by present a face. By manipulating the behavior associated with the face, participants can form an impression of the person behind the face and judge it as trustworthy or not. After getting such impression, participants then finished a attention blink task where the faces were used as stimuli. 

Another approach used indirect learning, where participants first associate visual stimuli (e.g., faces) with moral behaviors, then perform a task that examine the differences between faces associated with different moral behaviors. For example, @anderson_visual_2011 associated faces with different behaviors (both negative and neutral behaviors from both social and nonsocial domains) and then after participants to perform a binocular rivalry task, where a face and a building were presented to each eye and participant need to report the content of their vision by pressing buttons. They found that faces associated negative social behavior were dominant in the visual awareness for longer time than faces associated with other types of behaviors (but see @stein_no_2017). Similar paradigm were used to study trustworthiness of faces. In these studies, faces were associated with either sentences about trust related behavior (@eiserbeck_visual_2020). This indirect learning paradigm had been used not only for moral behaviors but also other affective meanings (Bliss-Moreau, Barrett, & Wright, 2008; Falvello, Vinson, Ferrari, & Todorov, 2015; Todorov & Olson, 2008). These studies showed that the building such association requires minimal behavioral information, abstract conceptual information are enough.

The studies reviewed above suggested the using associative learning task might be a valid way to study moral character in lab settings. In the current study, we further simplify the indirect learning paradigm by using geometric shapes as the visual stimuli instead of faces and moral character labels instead of behavior descriptions. Previous studies combined shapes and person label (e.g., self, friend, stranger) to study the self-referential effect and found a robust self-prioritization effect (@Sui_2012_JEPHPP). Here we modified the shape-label association paradigm to suit the moral character purpose. This paradigm has several advantages. first, it only used a few number of labels that represent different moral character, therefore control extra factors and avoid the semantic priming problem. Second, it use non-social visual stimuli, avoided the idiosyncratic features brought by using faces. Besides, the task itself is simple and can combined with other cognitive tasks. 

This paradigm consistent with the dynamic interactive model of person perception (@freeman_dynamic_2011). The dynamic interactive model of person perception proposed that the perceived personal traits are interactively linked with behavior and sensory stimuli. By activating some sensory stimuli, some person traits can be activated. The associative learning task reverse the process and directly linked the personal traits (moral traits in our case) with visual stimuli, therefore created a temporary but direct link between personal trait and visual stimuli.

[The current study] 
Using the person-label associative learning and perpetual matching task, the current study aimed at answering two questions mentioned above: (1) whether moral character related information are prioritized, if so, what is the exact pattern; (2) what is the potential explanation for such pattern.
...
Although the available studies agree that social/moral information can enhance the saliency of the sensory stimuli, yet the direction of the effect is not consistent. For instance, there are two studies reported a negativity effect where neutral faces associated with negative social behavioral were processed better than neutral faces that associated with neutral behaviors (@anderson_visual_2011, @eiserbeck_visual_2020, but see @stein_no_2017). The reason for the prioritization was usually attributed to affective meaning of the negative behaviors, which, in essence, is a threatening effect. In contrast, there was one study reported a positivity effect, where faces associated with positive interaction history were prioritized over faces associated with neutral or negative interaction history. And the positivity effect was attributed to a value-based information process (e.g., @shore_social_2013). 

The direction of the effect leads to different underlying explanations. The negativity effect usually explained by the evolutionary adoptive mechanism where the threatening feature were prioritized. However, accumulating evidence supported the view that negativity effect, especially those related to affective stimuli, are prioritized because of the low-level physical features, e.g., low frequency feature in the facial expression (). This is reflected in the pattern that threatening stimuli are prioritized in detection task, e.g., dot-probe task. The positivity effect, on the other hand, appeared later in the processing stage and were attributed to its rewarding value (we limit the value-based account to rewarding value). The value based account is an appealing explanation, there were strong evidence showing that positive stimuli are prioritized []. For example, [Anderson et al (2011)] found that stimuli associated with higher reward could be found more easily in a visual search task. The follow-up studies confirmed that value-based prioritization if a robust effect. A meta-analysis also found that positive stimuli are preferentially processed in perception []. In our experiments, the positive moral character label "good person" may represent a positive, vacuous value. The moral value of others was found to be profitable in previous survey [Abele et al].

When applying to social information such as moral character, both explanations ignored the social meaning of the stimuli. Increasing evidence supported the view that social categorization also impact our information processing, including perception [Xiao, Freeman]. Social categorization theory stated that we perceiving others based on whether or not belong to "us"[], as an extension of the self. In other words, we may view positive moral character as extension of the self, while bad people as them [person-based morality, group-based morality]. If moral judgment is a social categorization process [], and if social categorization impact our visual perception [], then we can infer positive moral character might be prioritized because we regard that as an natural extension of the self.

However, the above four experiments could not distinguish between these possibilities, because the concept "good-person" creating an unexpected ambiguity for the identity of the good person. it can both be rewarding and be categorized as in-group member, and previous studies using associative learning paradigm revealed that both rewarding stimuli [e.g., @Sui_2012_JEPHPP] and in-group information [@enock_overlap_2020] are prioritized.  
...
With limited cognitive resources, not all input information are processed equally. Salient visual stimuli, for example, are prioritized because of their physical features, affective value, or rewarding value [attention capture related papers]. In the social world, thus, important social information is also prioritized. For example, self-relevant and social affective information. Beyond that, recent studies found that moral related information also prioritized (@brady_attentional_2020; @gantman_moral_2014). The center of morality is person (). It is not surprising that moral character information that associated with person having advantage in early information processing stage (@eiserbeck_visual_2020, @anderson_visual_2011). Even instantly acquired social/moral information about a person can change the saliency of the visual stimuli of that person. For example, neutral faces associated with negative social behavioral were prioritized as compared to neutral (and positive) (@anderson_visual_2011, @eiserbeck_visual_2020, but see @stein_no_2017), neutral faces associated with trustworthy behaviors were processed faster in attention (@shore_social_2013). 

Although the available studies agree that social/moral information can enhance the saliency of the sensory stimuli, yet the direction of the effect is not consistent. For instance, there are two studies reported a negativity effect where neutral faces associated with negative social behavioral were processed better than neutral faces that associated with neutral behaviors (@anderson_visual_2011, @eiserbeck_visual_2020, but see @stein_no_2017). The reason for the prioritization was usually attributed to affective meaning of the negative behaviors, which, in essence, is a threatening effect. In contrast, there was a study reported a positivity effect, where faces associated with positive interaction history were prioritized over faces associated with neutral or negative interaction history. And the positivity effect was attributed to a value-based information process (e.g., @shore_social_2013). 

To understand the top-down influence of moral character on visual perception, we need first to find a robust effect that can be replicated. In the moral perceptual it is especially important because some of the experiments were not successfully replicated (see @stein_no_2017). After finding an stable effect, we could then explore the potential mechanisms. 
...
Two approaches were used to solve this issue. The first approach used direct learning. For example, @shore_social_2013 asked participants first interact with a person by present a face. By manipulating the behavior associated with the face, participants can form an impression of the person behind the face and judge it as trustworthy or not. After getting such impression, participants then finished a attention blink task where the faces were used as stimuli. 

Another approach used indirect learning, where participants first associate visual stimuli (e.g., faces) with moral behaviors, then perform a task that examine the differences between faces associated with different moral behaviors. For example, @anderson_visual_2011 associated faces with different behaviors (both negative and neutral behaviors from both social and nonsocial domains) and then after participants to perform a binocular rivalry task, where a face and a building were presented to each eye and participant need to report the content of their vision by pressing buttons. They found that faces associated negative social behavior were dominant in the visual awareness for longer time than faces associated with other types of behaviors (but see @stein_no_2017). Similar paradigm were used to study trustworthiness of faces. In these studies, faces were associated with either sentences about trust related behavior (@eiserbeck_visual_2020) or with experienced interaction in a trust game (@shore_social_2013). This paradigm had been used not only for moral behaviors but also other affective meanings and found that the building such association requires minimal behavioral information (Bliss-Moreau, Barrett, & Wright, 2008; Falvello, Vinson, Ferrari, & Todorov, 2015; Todorov & Olson, 2008).

The studies reviewed above suggested the using associative learning task might be a valid way to study moral character in lab settings. In the current study, we further simplify the paradigm by using geometric shapes as the visual stimuli instead of faces and use moral character words instead of sentences. Previous studies combined shapes and person label (e.g., self, friend, stranger) to study the self-referential effect and found a robust self-prioritization effect (@Sui_2012_JEPHPP). Here we modified the shape-person association paradigm to suit the moral character purpose. This paradigm has several advantages. first, it only used a few number of labels that represent different moral character, therefore control extra factors and avoid the semantic priming problem. Second, it use non-social visual stimuli, avoided the idiosyncratic features brought by using faces. Combined these two features, the task itself is simple and can combined with different cognitive tasks. 

This paradigm consistent with the dynamic interactive model of person perception (@freeman_dynamic_2011). The dynamic interactive model of person perception proposed that the perceived personal traits are interactively linked with behavior and sensory stimuli. By activating some sensory stimuli, some person traits can be activated. The associative learning task reverse the process and directly linked the personal traits (moral traits in our case) with visual stimuli, therefore created a temporary but direct link between personal trait and visual stimuli.

We tested the moral associative learning task by a series of experiments and found a robust effect moral positive effect.


### Good person as profitable other or as an extension of the self

After finding out a robust effect of moral positivity, the next question is how to reconcile our results with previous studies. As we mentioned, the results concerning the moral related perception studies already showed quite divergent results. While @gantman_moral_2014 found a valence effect of moral words without direction, @eiserbeck_visual_2020 reported a negative effect and @shore_social_2013 reported a positive effect. Note that @eiserbeck_visual_2020 only used negative and neutral stimuli, therefore it's difficult to tell whether there will be a positive effect if positive valence was also used. There are several possibilities for the positivity effect. 

The value based account is an appealing explanation, there were strong evidence showing that positive stimuli are prioritized []. For example, [Anderson et al (2011)] found that stimuli associated with higher reward could be found more easily in a visual search task. The follow-up studies confirmed that value-based prioritization if a robust effect. A meta-analysis also found that positive stimuli are preferentially processed in perception []. In our experiments, the positive moral character label "good person" may represent a positive, vacuous value. The moral value of others was found to be profitable in previous survey [Abele et al].

Another account that may explain this effect but paid less attention to is the social categorization theory. Social categorization theory stated that we perceiving others based on whether or not belong to "us"[], as an extension of the self. In other words, we may view positive moral character as extension of the self. Social categorization recently has been applied to moral psychology could be used to explain a variety of effect []. An independent research line had long found that social categorization impact our perceptual process [Xiao]. If moral judgment is a social categorization process [], and if social categorization impact our visual perception [], then we can infer positive moral character might be prioritized because we regard that as an natural extension of the self.

To test these two explanations, we designed the second series of experiments, where we have design in which the valence of the moral character and self-relevance were orthogonal. In these experiment, 
