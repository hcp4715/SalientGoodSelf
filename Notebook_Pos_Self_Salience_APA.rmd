...
df1a_1 <- read.csv(".\\exp1a\\rawdata_behav_exp1a_2014.csv", header = TRUE,
        ...
        dplyr::mutate(Subject = Subject + 1000)
...
df1b_1 <- read.csv(".\\exp1b\\rawdata_behav_exp1b_2014.csv", header = TRUE,
                   sep = ",", stringsAsFactors=FALSE, na.strings=c("","NA")) %>%
        dplyr::mutate(Site = "THU") %>%
        dplyr::mutate(Subject = Subject + 1100)
...
df1c <- read.csv(".\\exp1c\\rawdata_behav_exp1c_2014.csv", header = TRUE,
  ...
  dplyr::mutate(Site = "THU") %>%
  dplyr::mutate(Subject = Subject + 1200)     # change subject ID to 12XX
...
df2 <- read.csv(".\\exp2\\rawdata_behav_exp2.csv", header = TRUE,
  ...
  dplyr::mutate(Valence = ifelse(Valence == "Normal", "Neutral", Valence),       # recode values
                ...
                #Subject = factor(Subject)) %>%
  dplyr::mutate(Subject = ifelse(Subject < 40, Subject + 1000, Subject + 2000))  # recode subject ID, identify those particiant from exp1a
...
df3a <- read.csv(".\\exp3a\\rawdata_behav_exp3a_2014.csv", header = TRUE,
  ...
  dplyr::mutate(Valence = derivedFactor("Neutral" = (Shape == "Normalself" | Shape == 'Normalother'), 
                ...
                Age = ifelse(Age == 0, NA, Age)) %>%                             # if the min age is 0, that age is missing
                #Subject = factor(Subject)) %>%
  dplyr::mutate(Subject = Subject + 3100)                                        # re-code the subject id
...
df6a <- read.csv(".\\exp6a_erp1\\rawdata_erp_exp6a_2014.csv",header = TRUE, sep = ",",
  ...
  dplyr::mutate(Valence = ifelse(Valence == "Normal", "Neutral", Valence),                   # re-code the data
                ...
                Age = ifelse(Age == 0, NA, Age)) %>%
  dplyr::mutate(Site = "THU") %>%
  dplyr::mutate(Subject = Subject + 6100) 
...
Before analyzing, we first calculated the effect size based on for each participant. Then, we used the '' function of the metafor to estimate the overall effect size.

Specifically, we are interested in the effect size of the following effects. First, the effects of moral valence, regardless of identity. To this end, we estimated the different between good vs. bad, good vs. neutral, and neutral vs. bad, for both *d* prime and RTs. Second, the interaction between moral valence and identity. To this end, we estimated the effect for both self and other conditions. Additionally, we also examined the main effect of identity, regardless of valence. To this end we estimated the differences between self vs. other conditions, across all valence conditions.
