...
groupList_1 <- rep(c('Overall'), each = 3)
...
p_meta_list_1 = list() # create a empty list to store the forest plot
...
#df.res.meta_1_val <- df.res.meta_1[c(1:9, 14:22),]
#df.res.meta_1_val$EffectType <- rep(c('Good_Bad','Good_Neut','Neut_Bad'), 3*2)

p_meta_val_1 <- ggplot(df.res.meta_1, aes(x=DVtype, y=Cohen_d, fill=Effect)) + 
...
p_meta_val_2_self <- df.res.meta_2 %>%
  ...
  ggplot(., aes(x=DVtype, y=Cohen_d, fill=EffectType)) + 
  geom_bar(stat="identity", color=NA, 
           position=position_dodge()) +
  geom_errorbar(aes(ymin=Cohen_d - 1.96*se, ymax = Cohen_d + 1.96*se), width=.2,
                 position=position_dodge(.9)) +
