...
# General Methods
## Participants.
All experiments (1a ~ 6b, except experiment 3b) reported in the current study were first finished between 2014 to 2016 in Tsinghua University, Beijing, participants of these experiments were recruited in Tsinghua University community. To increase the power by adding more data so that each experiment has 50 or more valid data [@Simmons_2013_life], we recruited additional participants in Wenzhou University, Wenzhou, China in 2017. Due to the limited time and resources, additional data were collected for experiment 1a, 1b, 4a, and 4b, but not other experiments. We report all the variables measured in our experiments. In the meta-analysis, we also included the two experiment reported in @Hu_2019_GoodSelf.

 <!-- 21-word solution (Simmons, Nelson & Simonsohn, 2012; retrieved from http://ssrn.com/abstract=2160588) -->

## Design and Procedure
This series of experiment started to test the effect of instantly acquired moral valence on perceptual decision-making. For this purpose, we used the social associative learning paradigm (or self-tagging paradigm)[@Sui_2012_JEPHPP], in which participants first learn the associations between geometric shapes and labels of person with different moral valence (e.g., in first three studies, the triangle, square, and circle and good person, neutral person, and bad person, respectively). The associations of the shapes and label were counterbalanced across participants. After learning phase, participants finish a practice phase to familiar with the task, in which they viewed one of the shapes upon the fixation while one of the labels below the fixation and judged whether the shape and the label were matched. When participants can get 60% or higher accuracy at the end of the practicing session, they can start the experimental task which is the same as in the practice phase. Therefore, the design for first a few experiments were two (matchness: matched vs. mismatched) by three (moral valence: good vs. neutral vs. bad), and the dependent variables reported in this manuscript was reaction times and accuracy in the experimental phase, i.e., the perceptual matching task.

Across all experiment, experiment 1a, 1b, 1c, 2, and 6a shared the same 2 by 3 within-subject design. Of which the experiment 1a was the first experiment and 1b, 1c, and 2 were conducted to exclude other confounding variable's effect. More specifically, experiment 1b used different Chinese words as label to test whether the effect only occure with certain words. Experiment 1c manipulated the moral valence indirectly: participant first learnt to associate different moral behaviors with different neutral names, after remembered the association, they then performed the perceptual matching task by associating the names with different shapes. Experiment 2 tested whether the way we presented the stimuli influence the effect we observed by sequently presenting labels and shapes. Experiment 6a, which shared the same design as experiment 2, was an EEG experiment which aimed at exploring the neural correlates of the effect. But we will focus on the behavioral results of experiment 6a in this manuscript.

For experiment 3a, 3b, 4a, 4b, 6b, 7a, and 7b, we added self-referential as another within-subject variable. The experiment 3a directly extend the experiment 1a, with six conditions (good-self, neutral-self, bad-self, good-other, neutral-other, and bad-other) and six shapes (). The experiment 6b was an EEG experiment extended from experiment 6a by adding the independent variable self-reference. That's, the difference between 6b and 3a is that 6b presented label and shape sequentially. Because of the relatively high working memory load (six label-shape pairs), experiment 6b were conducted in two days: the first day participant remember the associations between label and shape and finished perceptual matching task, and the second day, they finished the task again while the EEG signals were recorded. To further test the effect of working memory load, experiment 3b was design to separate the self-referential trials and other-referential trials. That is, participant first two different blocks: for the self-reference blocks, they only response to good-self, neutral-self, and bad-self, with half of the trials was matched and half was not; for the other-reference blocks, they only reponded to good-other, neutral-other, and bad-other. Experiment 4a and 4b were design to test the automaticity of the binding between self/other and moral valence. In 4a, we used only two labels (self vs. other) and two shapes (), to manipulate the moral valence, we added label within the shape and instructed participants to ignore the presence of these moral related words. In 4b, we did the reverse: participant learnt three labels (good-person, neutral-person, and bad-person) and three shapes (), and the word "self" or "other" was presented in the shape. as in 4a, participants were told to ignore the words inside the shape. Experiment 7a and 7b are almost identical, they are designed to test the cross task robustness of the effect we observed in the aforementioned experiments. As we found reliable effect that the neutral and bad conditions constantly show nonsignificant results, we only used two conditions in moral valence, i.e., good vs. bad.
...
## Data analysis
We reported all the measurements, analyses, and results in all the experiments in the current study. All data were first pre-processed using `r cite_r("r-references.bib")`. The clean data were analyzed using JASP [0.8.6.0, www.jasp-stats.org, @Love_etal_2019_JASP]. Participants whose overall accuracy lower than 60% were excluded from analysis. Also, the accurate responses with less than 200ms reaction times were excluded from the analysis.

We analyzed accuracy performance using a signal detection approach, as in @Sui_2012_JEPHPP. The performance in each match condition was combined with that in the nonmatching condition with the same shape to form a measure of *d’*. Trials without response were coded either as “miss” (matched trials) or “false alarm” (mismatched trials). The *d’* were then analyzed using repeated measures analyses of variance (repeated measures ANOVA). 

The reaction times of accurate trials were also analyzed using repeated measures ANOVA. To control the false positive when conducting the post-hoc comparisons, we used Bonferroni correction. Please note that in the first two experiment (experiment 1a and 1b), we included the variable matchness (matched vs. mismatched) in our ANOVA of reaction times and then examine matched trials and mismatched trials separately when the interaction between matchness and other variables are significant. In most experiments, we found significant interaction between matchness and valence. Then, as previous study, we focused on the matched trial for the rest of the experiment [@Sui_2012_JEPHPP]. 

We reported the effect size of repeated measures ANOVA (omega squared) [@Bakeman_2015_eff_size; @Lakens_2013]. Also, we reported Cohen’s *d* and its 95% confidence intervals for the post-hoc comparisons. To provide more information about the results, we also reported the Bayes Factor using JASP [@Hu_2018_JASP; @Wagenmakers_2018_JASP]. The Bayes factor is the ratio of the probability of the current data pattern under alternative hypothesis (H_1) and the probability of the current data pattern under null hypothesis (H_0), which index the relative evidence for these two hypotheses from the current data. The BF_10 represents the evidence for alternative hypothesis (H_1) vs. evidence for null hypothesis (H_0); in contrast, BF_01 represents that evidence for null hypothesis over the evidence for althernative hypothesis. We used the default prior in JASP for all the Bayes Factor analyses, and used @Jeffreys_1961’s convention for the strength of evidence: the BF_10 > 3 means there are some evidence for H_1 as compared with H_0,  BF_10 great or equal to 10 means strong evidence for H_1.
...
# # there are 4 foreign students who might not be native Chinese speakers, but we didn't exclude them:
...
tmp <- df1a %>% 
  ...
  dplyr::group_by(Valence, Shape) %>% dplyr::summarise(N = length(unique(Subject))) %>% arrange(N)
