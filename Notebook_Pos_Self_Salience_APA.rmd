...
CommonColnames_d  <- c("ExpID", "Site", "Subject", "Age", "Sex", 'Domain', "Identity", "Val_sh", "dprime")
CommonColnames_rt <- c("ExpID", "Site", "Subject", "Age", "Sex", 'Domain', "Matchness", "Identity", "Val_sh", "RT")
```
...
tmp <- df1a %>% 
  ...
  dplyr::group_by(Val_sh, Shape) %>% dplyr::summarise(N = length(unique(Subject))) %>% arrange(N)
...
df1a.v.dprime_l <- df1a.v %>%
  ...
  dplyr::group_by(Site, Subject, Age, Sex, Val_sh, sdt) %>%
  ...
  dplyr::select(Site, Subject, Age, Sex, Val_sh, dprime) %>%                 # select relevant columns
  dplyr::mutate(Val_sh = factor(Val_sh, levels = c("Good","Neutral","Bad")))


# get the mean RT for each particpant and each condition for ANOVA
df1a.v.rt_m <- df1a.v %>%
  dplyr::filter(ACC == 1) %>%
  dplyr::group_by(Site,Subject, Age, Sex, Matchness, Val_sh) %>%
...
# check prior
prior_null_int <- exp(rnorm(n=1e5, mean=-1, sd=0.5))  # chosen intercept prior
prior_null_sig <- exp(rnorm(n=1e5, mean=0.4, sd=0.3))      # chosen intercept prior
prior_null_b <- exp(rnorm(n=1e5, mean=0, sd=0.5))          # chosen intercept prior

HDI95_null_int <- unname(quantile(prior_null_int, c(.025, .975)))

ggplot() + geom_density(aes(x = prior_null_int)) + xlim(-5, 5) + geom_vline(xintercept = HDI95_null_int[1]) +  geom_vline(xintercept = HDI95_null_int[2])
...
modelposterior <- as.mcmc(m6) # with the as.mcmc() command we can use all the CODA package convergence statistics and plotting options
gelman.diag(modelposterior[, 1:8])

gelman.plot(modelposterior[, 1:8])
...
m7 <- brms::brm(formula = bf(RT ~ Val_sh + Matchness + Val_sh:Matchness + (1|Subject), 
                             ndt ~ Subject), 
                data = df1a.v.rt, 
                family = shifted_lognormal(),
                chains = 4, cores = 4, warmup = 3000, iter = 5000,
                prior = prior_int,
                file = 'exp1a_int_log')

#m7 <- brms::brm(RT ~ Val_sh + Matchness + Val_sh:Matchness + (1|Subject), 
#                data = df1a.v.rt, 
#                family = shifted_lognormal(),
#                chains = 4, cores = 4, warmup = 3000, iter = 5000,
#                prior = prior_int,
...
df1a_dprime_anova <- afex::aov_ez('Subject','dprime',df1a.v.dprime_l, within = c('Val_sh'))
df1a_dprime_anova_apa <- df1a_dprime_anova$aov %>% papaja::apa_print()

posthoc_1a_d <- emmeans::emmeans(df1a_dprime_anova, "Val_sh") # compare each valence for both self and other condition
#pairs(posthoc_1a_d)
```
...
df1a_RT_anova <- afex::aov_ez('Subject','RT_m',df1a.v.rt_m, within = c('Matchness','Val_sh')) # using afex's function

df1a_RT_anova_apa <- df1a_RT_anova %>% papaja::apa_print()

df1a.v.rt_m1 <- df1a.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match") %>%
  dplyr::group_by(Site,Subject,Matchness, Val_sh) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(Val_sh = factor(Val_sh, levels = c("Good","Neutral","Bad")))

df1a_RT_anova_m <- afex::aov_ez('Subject','RT_m',df1a.v.rt_m1, within = c('Val_sh'))
...
df1a_RT_anova_nm <- afex::aov_ez('Subject','RT_m',df1a.v.rt_m2, within = c('Val_sh'))

posthoc_1a_rt <- emmeans::emmeans(df1a_RT_anova_m, "Val_sh") # compare each valence for both self and other condition
# pairs(posthoc_1a_rt)
...
SlfEstAlpha <-  psych::alpha(df.scales.v[,SESNames], keys=SESKeys)  # calculate the alpha coefficient of SES
SESOmega <- psych::omega(df.scales.v[,SESNames])  
...
# get the parameters from hddm
df1b.v.hddm_params <- read.csv(".\\HDDM\\exp1b_hddm_params.csv",header = TRUE, sep = ",",
                 stringsAsFactors=FALSE,na.strings=c("","NA")) %>%
  dplyr::rename(Subject = subj_idx, Matchness = match, Valence = val) %>%
  dplyr::select(Subject, Matchness, Valence, knode_name, mean) %>%
  tidyr::drop_na() %>%
...
df1a.v.cor.anal <- df1a.v.cor.anal %>% dplyr::right_join(., df1a.v.hddm_params, by = 'Subject') # merge to cor dataframe
