...
One would argue that the effect here may represent an effect in memory or in decision-making process instead of perpetual effect *per se*. We agree that perceptual decision-making process include more processes than just perception, if we define perception narrowly as encoding of the sensory information. This suggest that we need to apply computational models to behavioral data and/or combined with data from other modules to quantify the processes underlying perceptual decisions. For example, sequential sampling models suggest that, when making a perceptual decision, the agent is continuously accumulate evidence until the amount of evidence passed a threshold, then a decision is made. In these models, the evidence, or decision variable, can accumulate from both sensory information but also memory []. Recently applications of sequential sample model to perceptual matching tasks also suggest that different processes may contributed to the prioritization effect of self [] or moral self []. Moreover, reinforcement learning models also revealed that ....
