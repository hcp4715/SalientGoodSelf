...
CommonColnames <- c("ExpID", "Site", "Subject", "Age", "Handedness", "Sex", "BlockNo", "TrialNo", 
                    'Domain',"Matchness", "Identity", "Valence", "CRESP", "RESP", "ACC", "RT")
```
...
df3b <- read.csv(".\\exp3b\\rawdata_behav_exp3b_2017.csv", header = TRUE,
  ...
  dplyr::mutate(Valence = derivedFactor("Neutral" = (Shape == "Neutralself" | Shape == 'NeutralOther'), 
                ...
                Site = "WZU", ExpID = "Exp3b", Domain = "Morality") %>%
  dplyr::select(CommonColnames)
...
df5.v.rt_m <- df5.v %>%
  dplyr::filter(ACC == 1) %>%
  dplyr::group_by(Site,Subject,Matchness,taskType,Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(Valence = factor(Valence, levels = c("Good","Neutral","Bad")),
...
```{r combine Data,echo=FALSE,results='hide'}
df.meta <- rbind(df1a,df1b,df1c,df2,df3a,df4a,df5_meta,df6a_meta,df6b_meta) # combine data, need to exclude those overlapping participants.
