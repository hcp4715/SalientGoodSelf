...
pkgNeeded <- (c('papaja',"tidyverse", 'metafor',
                "corrplot","readr", 'patchwork',
                "Hmisc",'mosaic', 'here'))
...
The raw data were pre-processed using the `tidyverse` package of r, in which we excluded the practicing trials, invalid trials of each participants, and invalid participant, if there was any `r cite_r("r-references.bib")`. Individual experiment's results were analyzed as in @Sui_2012_JEPHPP, see supplementary materials. More specifically, we analyzed the accuracy performance using a signal detection approach, in which the performance in each match condition was combined with that in the nonmatching condition with the same shape to form a measure of *d’*. Trials without response were coded either as “miss” (matched trials) or “false alarm” (mismatched trials). The *d’* were then analyzed using repeated measures analyses of variance (repeated measures ANOVA). The reaction times of accurate trials were also analyzed using repeated measures ANOVA. To control the false positive rate when conducting the post-hoc comparisons, we used Bonferroni correction. See supplementary materials for the results of each experiment's method and results, which included the significance test results, effect size [@Bakeman_2015_eff_size; @Lakens_2013], and Bayes factor calculated by JASP [@Hu_2018_JASP; @Wagenmakers_2018_JASP]. 
...
For the first two parts, we synthesized results from individual results using mini-meta-analysis [@Goh_2016_mini]. The Mini meta-analyses were carried out by using the `metafor` package (Viechtbauer, 2010). We first calculated the mean of *d*' and RT of each condition for each participant, then calculate the effect size (Cohen's *d*) and variance of the effect size for all contrast we interested: Good v. Bad, Good v. Neutral, and Neutral v. Bad for the effect of valence, and self vs. other for the effect of self-relevance. Cohen's *d* and its variance were estimated using the following formula [@Cooper_2009_handbook]:
...
The DDM analyses were finished by HDDM[@wiecki_hddm_2013], as reported in  @Hu_2020_GoodSelf. That is, we used the response code approach, matched response were coded as 1 and mismatched responses were coded as 0. To fully explore all parameters, we allow all four parameters of DDM free to vary. We then extracted the estimation of all the four parameters for each participants for the correlation analyses.
...
df.ES_4b <- data.frame(matrix(nrow=length(unique(df4b.meta.d$ExpID))*length(effectList_2)*2, ncol=0)) %>%
  dplyr::mutate(DVtype = rep(c('RT', 'dprime'), each = length(unique(df4b.meta.d$ExpID))*length(effectList_2)),
                ExpID  = rep(rep(unique(df4b.meta.d$ExpID), each = length(effectList_2)), 2),
                Effect = rep(effectList_2, length(unique(df4b.meta.d$ExpID))*2),
                M1 = NA, SD1 = NA, M2 = NA, SD2 = NA, N = NA, r = NA, ES = NA, ES.var = NA)
...
```{r 'plot-exp4b-effect', fig.cap="Effect size (Cohen's *d*) of Valence in Exp4b.", warning=FALSE}
# df.ES_4b %>%
#   #dplyr::filter(Identity == "Self") %>%
#   ggplot(., aes(x=EffectType, y=ES)) + # , color=Val, fill=Val 
#   geom_pointrange(aes(ymin=ES - 1.96*sqrt(ES.var), ymax = ES + 1.96*sqrt(ES.var)), 
#                   position = position_dodge(width = 0.5),
#                   shape=18, size=1.5) +
#   geom_hline(yintercept=0, size=1, color='grey', linetype = 'dashed') +
...
df.ES_4b_diff <- df.ES_4b %>%
  dplyr::select(Identity, EffectType, DVtype, ES, ES.var) %>%
  tidyr::pivot_wider(., values_from = c(ES,ES.var), names_from = Identity) %>%
  dplyr::mutate(diff_diff = `ES_Self-ref.` - `ES_Other-ref.`,
                diff_var = sqrt((`ES.var_Self-ref.`^2 +  `ES.var_Other-ref.`^2 )/2)) %>%
  dplyr::select(EffectType, DVtype, diff_diff, diff_var) %>%
  dplyr::rename(ES = diff_diff, ES.var = diff_var) 
...
```{r 'plot-exp5-effect', fig.cap="Effect size (Cohen's *d*) of Valence in Exp4b.", warning=FALSE}
df.ES_5 %>%
...
# sapply(df.corr_norm, class)

# library(corrr)
res.cor <- df.corr_norm %>%
...
# res.cor_bayes <- df.corr_norm %>%
#   dplyr::select(-c(ExpID:Sex)) %>%
#   correlation::correlation(., bayesian = T, bayesian_test = 'bf')

# cor_pairs_bayes <- res.cor_bayes %>%
#  dplyr::filter(Parameter1 %in% colnames(df.q_scores.v)) %>%
#   dplyr::filter(Parameter2 %in% colnames(df.meta_all_wide[7:24])) %>%
#   dplyr::filter((BF <= 1/6)| (BF >= 3)) %>%
#  dplyr::arrange(BF)

# bootstrap for all
boot_plot_list <- list()
corr_plot_list <- list()
for (row_id in 1:nrow(cor_pairs)){
  var1 <- df.corr_norm %>% dplyr::select(cor_pairs$Parameter1[row_id])
  var2 <- df.corr_norm %>% dplyr::select(cor_pairs$Parameter2[row_id])
  var_tmp <- data.frame(var1, var2) %>%
    tidyr::drop_na() %>%
    bootES::bootES(., R = 4000, effect.type = 'r')
  cor_pairs$BootES_cor[row_id] <- var_tmp$t0[1]
  cor_pairs$BootES_lb[row_id] <- var_tmp$bounds[[1]]
  cor_pairs$BootES_ub[row_id] <- var_tmp$bounds[[2]]
  
  corr_plot_list[[row_id]] <- data.frame(var1, var2) %>%
    ggplot(., aes_string(x = cor_pairs$Parameter1[row_id], y = cor_pairs$Parameter2[row_id])) + 
    geom_point() + 
    geom_smooth(method=lm) +
    labs(title=paste(cor_pairs$Parameter1[row_id], '&', cor_pairs$Parameter2[row_id], sep = ' '), 
       x = cor_pairs$Parameter1[row_id], 
       y = cor_pairs$Parameter2[row_id]) +
    apatheme
  
  boot_plot_list[[row_id]] <- var_tmp$t %>% as.data.frame(.) %>% 
    dplyr::rename(r = V1) %>%
    ...
    labs(title=paste("Bootstr 4k |", cor_pairs$Parameter1[row_id], '&', cor_pairs$Parameter2[row_id], sep = ' '), 
         x="Correlation", y = "Count") +
    apatheme
}

# bf = BayesFactor::correlationBF(x = df.corr_norm$Good_v, y = df.corr_norm$mrlslfImg)
# bf
# samples = BayesFactor::posterior(bf, iterations = 10000)
# summary(samples)
# plot(samples[,"rho"])
```
...
```{r plot-corr-1, fig.cap="Correlation between moral self-image, self esteem, and behavior", fig.width=8, warning=FALSE}
library(patchwork)
corr_plot_list[[1]] + corr_plot_list[[2]] + corr_plot_list[[3]] + corr_plot_list[[6]] + 
           boot_plot_list[[1]] + boot_plot_list[[2]] + boot_plot_list[[3]] + boot_plot_list[[6]] + plot_layout(ncol = 4)
```
...
```{r plot-corr-2, fig.cap="Correlation between moral identity and behavior", fig.width=6, warning=FALSE}
library(patchwork)
corr_plot_list[[4]] + corr_plot_list[[5]] + corr_plot_list[[7]] +
  boot_plot_list[[4]] + boot_plot_list[[5]] + boot_plot_list[[7]] + plot_layout(ncol = 3)
```
