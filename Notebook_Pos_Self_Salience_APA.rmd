...
library(patchwork)
p1 <- p_exp4a_rt1 + exp4a_sdt_p + plot_annotation(tag_levels = 'A')  + plot_layout(nrow = 1, byrow = FALSE, guides = "collect") 
...
library(patchwork)
p1 <- p_exp4b_rt1 + exp4b_sdt_p + plot_annotation(tag_levels = 'A')  + plot_layout(nrow = 1, byrow = FALSE, guides = "collect") 
p2 <- p_exp4b_rt_diff_val + p_exp4b_dprime1_diff_val + plot_layout(nrow = 1, byrow = FALSE, guides = "collect") 
p3 <- p_exp4b_rt_diff_id + p_exp4b_dprime1_diff_id + plot_layout(nrow = 1, byrow = FALSE, guides = "collect") 
...
For the task relevant part, we found shapes associated with good person conditions were performed better than other-related conditions, on both *d* prime and reaction times.

Most importantly, we found evidence, that task-irrelevant self-relevance also played an role. For shapes associated with good person, the *d* prime was greater when shapes had an "self" inside as task-irrelevant stimuli than with "other" inside () or bad self (), but this effect did not happen whent the target shape where associated with "neutral" or "bad" person. The same trend appear for the RT data. See Figure \@ref(fig:plot-exp4b-BGLM).
