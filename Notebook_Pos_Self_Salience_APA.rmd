...
# Normalize the person distance by dividing the maxiumum value for each data point
df.perdist <- df.scales %>%
  ...
  dplyr::mutate(maxDist = pmax(!!!rlang::syms(perDistNames))) 

df.perdist[,3:31] <- df.perdist[,3:31]/df.perdist$maxDist

df.perdist <- df.perdist %>%         # find the maximum value of the row
    dplyr::mutate(# sumRaw = rowMeans(.[3:31], na.rm = T),
                SelfGood_pd = rowMeans(.[grep("SelfGood", names(.))], na.rm = T),
                SelfNeut_pd = rowMeans(.[grep("SelfNeut", names(.))], na.rm = T),
                SelfBad_pd  = rowMeans(.[grep("SelfBad", names(.))], na.rm = T),
                GoodNeut_pd = rowMeans(.[grep("GoodNeut", names(.))], na.rm = T),
                GoodBad_pd  = rowMeans(.[grep("GoodBad", names(.))], na.rm = T),
                NeutBad_pd  = rowMeans(.[grep("NeutBad", names(.))], na.rm = T)) %>%
    dplyr::select(expID, subjID,
                SelfGood_pd, SelfNeut_pd, SelfBad_pd, GoodNeut_pd, GoodBad_pd, NeutBad_pd) %>%
  tidyr::drop_na()
...
df.scales <- df.scales %>%
  dplyr::mutate(morId_5_r = 0 - morId_5,
                SES3_r = 5 - SES3,
                SES5_r = 5 - SES5,
                SES9_r = 5 - SES9,
                SES10_r = 5 - SES10,
                SlfEst_sum = rowSums(.[SlfEstNames_r]),
...
```{r test_SEM, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# Test the SEM, compare two models of the moral identity scale

mrlID.m1 <- 'mrlId =~ morId_1 + 
morId_2 +
morId_5 +
morId_8 +
morId_10 +
...
# regression
Good ~ mrlIdInt + mrlIdExt 
'

sem.m2 <- 'mrlIdInt =~ morId_1 + 
morId_2 +
morId_5 +
morId_8 +
morId_10 +
...
# regression
Good ~ mrlIdInt + mrlIdExt + morSlfImg
Bad ~ mrlIdInt + mrlIdExt + morSlfImg

# correlations
Good ~~ Bad
'

fit.mrlID.m1 <- lavaan::cfa(mrlID.m1, data = df.scales, estimator = "MLR")
fit.mrlID.m2 <- lavaan::cfa(mrlID.m2, data = df.scales, estimator = "MLR")

fit.sem.m1 <- lavaan::sem(sem.m1, data = tmp, estimator = "MLM")
fit.sem.m2 <- lavaan::sem(sem.m2, data = tmp, estimator = "MLM")

#Model Outputs
summary(fit.mrlID.m1, standardize = TRUE, fit.measures=TRUE)
summary(fit.mrlID.m2, standardize = TRUE, fit.measures=TRUE)

summary(fit.sem.m1, standardize = TRUE, fit.measures=TRUE)
summary(fit.sem.m2, standardize = TRUE, fit.measures=TRUE)

sem.comp <- lavaan::anova(fit.sem.m1, fit.sem.m2)
sem.comp

# plot the path
# semPlot::semPaths(fit.mrlID.m1, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
semPlot::semPaths(fit.mrlID.m2, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)

semPlot::semPaths(fit.sem.m1, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
semPlot::semPaths(fit.sem.m2, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
```
...
# boxplot
df.perdist %>% 
  tidyr::pivot_longer(., cols = SelfGood_pd:NeutBad_pd,
                      names_to = 'PerDist',
                      values_to = 'value') %>%
...
mean(df.perdist$GoodBad_pd) - 2.5*sd(df.perdist$GoodBad_pd)

df.plot3 <- df.perdist %>%
  dplyr::mutate(sum_GN_NB = GoodNeut + NeutBad,
                sum_SG_SB = SelfGood + SelfBad,
                sum_SG_SB_m = SelfGood + SelfBad - SelfNeut)
df.plot3 %>%
  tidyr::pivot_longer(., cols = GoodNeut_NonSelf:GoodBad_Self,
                      names_to = 'PerDist',
                      values_to = 'value') %>%
  tidyr::separate(., col = PerDist, into = c('dist', 'ID'), sep = '_', remove=TRUE) 
  ggplot2::ggplot(., aes(x=Self, y=NonSelf, color=ID, shape=ID)) +
  geom_point() + 
  geom_smooth(method=lm)

df.plot3 %>% 
  ggplot(., aes(x = GoodBad, y = sum_GN_NB)) +
   geom_point() + 
  geom_smooth(method=lm)

df.plot3 %>% 
  ggplot(., aes(x = GoodBad, y = sum_SG_SB)) +
   geom_point() + 
  geom_smooth(method=lm)

df.plot3 %>% 
  ggplot(., aes(x = GoodBad, y = sum_SG_SB_m)) +
   geom_point() + 
  geom_smooth(method=lm)
  
df.plot2 <- df.perdist %>%
  # dplyr::select(-c(SelfSelf, SelfStra)) %>%
  dplyr::mutate(GoodNeut_Self = abs(SelfGood - SelfNeut),
                NeutBad_Self = abs(SelfBad - SelfNeut),
                GoodBad_Self = abs(SelfBad - SelfGood)) %>%
  dplyr::select(-c(SelfGood, SelfNeut, SelfBad)) %>%
  dplyr::rename(GoodNeut_NonSelf = GoodNeut,
                GoodBad_NonSelf = GoodBad,
                NeutBad_NonSelf = NeutBad)

# correlation::correlation(df.plot2[,2:7])
...
df.plot2.sum_p <- summarySE(df.plot2, measurevar = "value", groupvars = c('ID', 'dist')) 

# group the scatter point.
df.plot2 %>%
  ggplot2::ggplot(., aes(x=Self, y=NonSelf, color=dist, shape=dist)) +
  geom_point() + 
  geom_smooth(method=lm)
...
df.plot2 <- df.plot2 %>%
    dplyr::mutate(dist =factor(dist, levels = c('GoodNeut', 'NeutBad', 'GoodBad')),
                  ID = factor(ID, levels = c("Self", 'NonSelf')),
                  # DVs = factor(DVs, levels = c('RT', 'dprime')),
                  # create an extra column for plotting the individual data cross different conditions.
                  Conds = mosaic::derivedFactor("0.9" = (dist == 'GoodNeut' & ID == 'Self'),
                                                "1.1" = (dist == 'GoodNeut' & ID == 'NonSelf'),
                                                "1.9" = (dist == 'NeutBad' & ID == 'Self'),
                                                "2.1" = (dist == 'NeutBad' & ID == 'NonSelf'),
                                                "2.9" = (dist == 'GoodBad' & ID == 'Self'),
                                                "3.1" = (dist == 'GoodBad' & ID == 'NonSelf'),
                                                method ="first", .default = NA),
                  Conds = as.numeric(as.character(Conds)),
    ) 
df.plot2$Conds_j <- jitter(df.plot2$Conds, amount=.03) # add gitter to x
...
df.plot2.sum_p <- summarySE(df.plot2, measurevar = "value", groupvars = c('dist', 'ID')) %>%
    dplyr::mutate(Cond_num = mosaic::derivedFactor("1"  = (dist == 'GoodNeut'),
                                                "2" = (dist == 'NeutBad'),
                                                "3" = (dist == 'GoodBad'),
                                                method ="first", .default = NA),
...
df.plot2  %>% # dplyr::filter(DVs== 'RT') %>%
    ...
    #          linetype = 1, size = 1, colour = "#000000", alpha = 0.01) + 
    geom_point(aes(x = Conds_j, y = value, group = Subject, colour = as.factor(ID)),   # plot individual points
               #colour = "#000000", 
               size = 1, shape = 20, alpha = 0.15) +
    geom_line(data = df.plot2.sum_p, aes(x = as.numeric(dist), # plot the group means  
                                        y = value, 
                                        group = ID, 
                                        colour = as.factor(ID),
    ), 
    linetype = 1, position = pd1, size = 2) +
    geom_point(data = df.plot2.sum_p, aes(x = as.numeric(dist), # group mean
                                         y = value, 
                                         group = ID, 
                                         colour = as.factor(ID),
    ), 
    shape = 18, position = pd1, size = 5) +
    geom_errorbar(data = df.plot2.sum_p, aes(x = as.numeric(dist),  # group error bar.
                                            y = value, 
                                            group = ID, 
                                            colour = as.factor(ID),
                                            ymin = value- 1.96*se, 
                                            ymax = value+ 1.96*se), 
                  width = .05, position = pd1, size = 1, alpha = 0.75) +
    scale_colour_brewer(palette = "Dark2") +
    scale_x_continuous(breaks=c(1, 2, 3),
                       labels=c("GoodNeut", "NeutBad", "GoodBad")) +
    scale_fill_brewer(palette = "Dark2") +
    theme_bw() +
...
fit.dist.m1 <- lavaan::cfa(dist.m1, data = df.perdist, estimator = "MLR")
fit.dist.m2 <- lavaan::cfa(dist.m2, data = df.perdist, estimator = "MLR")

summary(fit.dist.m1, standardize = TRUE, fit.measures=TRUE)
summary(fit.dist.m2, standardize = TRUE, fit.measures=TRUE)

semPlot::semPaths(fit.dist.m2, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
```
...
### failed attempt:
# We tried to first correlate the DDM parameters and the questionnaire separately for self-referential and other-referential, and then validation the correlation using data of experiment without self/other-referential. But the sample size for the correlation between questionnaires and behavioral results from experiment without identity are too small n = 16. I will record the code as a commit and delete those exploration later.

### Step 1: calculate the correlation of self-referential and other-referential in the later part.
...
subj.common_id <- intersect(df.scale.fs$subjID, unique(df_hddm_param_2$Subject))  # 183

# df.scales.v_id <- df.scales %>% dplyr::filter(subjID %in% subj.common_id) %>%
#  dplyr::select_if(~sum(!is.na(.)) > 0) %>% # remove columns that only have NA.
#  dplyr::select(c('subjID'), mrlIdIntNames, mrlIdExtNames, SlfEstNames, mrlslfImgNames) %>%
#  dplyr::left_join(., df.perdist, by = "subjID") # %>%
  # dplyr::filter_all(any_vars(!is.na(.[, -1])))

df.corr.id <- df_hddm_param_2 %>%
  dplyr::filter(Subject %in% subj.common_id) %>%
  dplyr::left_join(., df.scale.fs, by = c('Subject'='subjID')) %>%
  dplyr::select(-expID)

df.corr.id_s <- df.corr.id %>%
  dplyr::filter(Identity == 'Self') %>%
  dplyr::select(-c(Subject, Identity, morSlfImg, mrlIdExt, mrlIdInt, SlfEst)) %>%
  #tidyr::drop_na() %>%  # N = 50
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0.15, missing = 'listwise')

#colnames(df.corr.id)
#summary(df.corr.id.1)

# plot(df.corr.id.1, layout = 'spring')

df.corr.id_o <- df.corr.id %>%
  dplyr::filter(Identity == 'Other') %>%
  dplyr::select(-c(Subject, Identity, morSlfImg, mrlIdExt, mrlIdInt, SlfEst)) %>%
  #tidyr::drop_na() %>%
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0.15, missing = 'listwise')
  #bootnet::bootnet_EBICglasso(., tuning = 0.5, missing = 'listwise')

# colnames(df.corr.id.2)
summary(df.corr.id_o)

Layout1 <- qgraph::averageLayout(df.corr.id_s, df.corr.id_o)

plot(df.corr.id_s, layout = Layout1)
plot(df.corr.id_o, layout = Layout1)

df.corr.id.3 <- df.corr.id %>%
  dplyr::filter(Identity == 'Self') %>%
  dplyr::select(-c(Subject, Identity, SelfGood_pd, SelfNeut_pd, SelfBad_pd, GoodNeut_pd, GoodBad_pd, NeutBad_pd)) %>%
  #dplyr::select(-tidyselect::one_of(perDistNames)) %>%
  # tidyr::drop_na() %>% # n = 55
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0, missing = 'listwise')
  #bootnet::bootnet_EBICglasso(., tuning = 0.5, missing = 'listwise')

#colnames(df.corr.id.2)
summary(df.corr.id.3)
#plot(df.corr.id.3, layout = 'spring')

df.corr.id.4 <- df.corr.id %>%
  dplyr::filter(Identity == 'Other') %>%
  dplyr::select(-c(Subject, Identity, SelfGood_pd, SelfNeut_pd, SelfBad_pd, GoodNeut_pd, GoodBad_pd, NeutBad_pd)) %>%
  #dplyr::select(-dplyr::starts_with('Self_')) %>%
  #dplyr::select(-tidyselect::one_of(perDistNames)) %>%
  #tidyr::drop_na() %>%
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0.1, missing = 'listwise')
  #bootnet::bootnet_EBICglasso(., tuning = 0.5, missing = 'listwise')

#colnames(df.corr.id.2)
summary(df.corr.id.4)

Layout2 <- qgraph::averageLayout(df.corr.id.3, df.corr.id.4)

plot(df.corr.id.3, layout = Layout2, maximum=0.47)
plot(df.corr.id.4, layout = Layout2, maximum=0.47)

subj.common.no_id <- intersect(df.scale.fs$subjID, unique(df_hddm_param_1$Subject))  # 77?

df.corr.no_id <- df_hddm_param_1 %>%
  dplyr::filter(Subject %in% subj.common.no_id) %>%
  dplyr::left_join(., df.scale.fs, by = c('Subject'='subjID')) %>%
  dplyr::select(-expID)

df.corr.no_id.1 <- df.corr.no_id %>%
  dplyr::select(-c(Subject, morSlfImg, mrlIdExt, mrlIdInt, SlfEst)) %>%
  #dplyr::select(-dplyr::starts_with('Other_')) %>%
  #tidyr::drop_na() %>% # n =74
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0.15, missing = 'listwise')

#colnames(df.corr.id)
summary(df.corr.no_id.1)

Layout1 <- qgraph::averageLayout(df.corr.id_s, df.corr.id_o, df.corr.no_id.1)

plot(df.corr.no_id.1, layout = Layout1)

df.corr.no_id.2 <- df.corr.no_id %>%
  dplyr::select(-c(Subject, SelfGood_pd, SelfNeut_pd, SelfBad_pd, GoodNeut_pd, GoodBad_pd, NeutBad_pd)) %>%
  #tidyr::drop_na() %>% # n = 16, here is the problem!
  #dplyr::select(-dplyr::starts_with('Other_')) %>%
  bootnet::estimateNetwork(., default = "EBICglasso", tuning = 0, missing = 'listwise')

#colnames(df.corr.id.2)
summary(df.corr.no_id.2)

Layout2 <- qgraph::averageLayout(df.corr.id.3, df.corr.id.4, df.corr.no_id.2)

plot(df.corr.no_id.2, layout = Layout2, maximum=0.47)

df.corr.all.1 <- df.corr.id %>%
  dplyr::filter(Identity == "Self") %>%
  dplyr::select(-Identity) %>%
  base::rbind(., df.corr.no_id) %>%
  dplyr::select(-c(Subject, SelfGood_pd, SelfNeut_pd, SelfBad_pd, GoodNeut_pd, GoodBad_pd, NeutBad_pd)) %>%
  #tidyr::drop_na() %>% # n = 71, here is the problem!
  #dplyr::select(-dplyr::starts_with('Other_')) %>%
  bootnet::estimateNetwork(., default = "EBICglasso", corMethod = 'cor_auto', tuning = 0, missing = 'listwise')

summary(df.corr.all.1)
plot(df.corr.all.1, layout = 'spring', maximum=0.47)

res.cor.id <- df.corr.id.fs %>%
  dplyr::select(-Subject) %>%
  correlation::correlation(., method="pearson", p_adjust = 'none')


df.corr.all.2 <- df.corr.id %>%
  dplyr::filter(Identity == "Self") %>%
  dplyr::select(-Identity) %>%
  base::rbind(., df.corr.no_id) %>%
  dplyr::select(-c(Subject, morSlfImg, mrlIdExt, mrlIdInt, SlfEst)) %>%
  #tidyr::drop_na() %>% # n = 124, here is the problem!
  #dplyr::select(-dplyr::starts_with('Other_')) %>%
  bootnet::estimateNetwork(., default = "EBICglasso", corMethod = 'cor_auto', tuning = 0, missing = 'listwise')

summary(df.corr.all.2)
plot(df.corr.all.2, layout = 'spring', maximum=0.47)
...
cor_pairs.id <- res.cor.id %>%
  dplyr::filter(Parameter1 %in% colnames(df.corr.id.fs[2:19])) %>%
  dplyr::filter(Parameter2 %in% colnames(df.corr.id.fs[20:29])) %>%
  dplyr::filter(p <= 0.1) %>%
  dplyr::arrange(p)

#summary(FS.m.id.fit2, standardize = TRUE, fit.measures=TRUE)
# plot the path
#semPlot::semPaths(FS.m.id.fit2, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
...
v =~ Self_Good_v + Self_Bad_v + Other_Good_v + Other_Bad_v

a =~ Self_Good_a + Self_Bad_a + Other_Good_a + Other_Bad_a

#Self_Badv =~ Self_Bad_v

#Self_Gooda =~ Self_Good_a

#Self_Bada =~ Self_Bad_a

#Other_Goodv =~ Other_Good_v

#Other_Badv =~ Other_Bad_v

#Other_Gooda =~ Other_Good_a

#Other_Bada =~ Other_Bad_a
...
# regression
Self_Good_v ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Self_Bad_v ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Self_Good_a ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Self_Bad_a ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Other_Good_v ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Other_Bad_v ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Other_Good_a ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst
Other_Bad_a ~ mrlIdInt + mrlIdExt + morSlfImg + SlfEst

# correlations
#Self_Good_v ~~ Self_Bad_v
#Self_Good_v ~~ Other_Good_v
#Other_Good_v ~~ Other_Bad_v
#Self_Bad_v ~~ Other_Bad_v
#Self_Good_a ~~ Self_Bad_a
#Self_Good_a ~~ Other_Good_a
#Other_Good_a ~~ Other_Bad_a
#Self_Bad_a ~~ Other_Bad_a
'

fit.sem.m1 <- lavaan::sem(sem.m1, data = df.corr.id, estimator = "MLR", missing = 'fiml', orthogonal=T)
fit.sem.m2 <- lavaan::sem(sem.m1, data = df.corr.id, estimator = "MLR", missing = 'listwise')
...
morId_16
  ...
  # regressions
    Self_Goodv ~ mrlIdInt + mrlIdExt
'

fit <- lavaan::fsr(fsr_m, data = df.corr.id, fsr.method = "Skrondal-Laake")
summary(fit)
...
# temp data for SEM
tmp <- merge(df.scales.v, df.meta_all_wide, by.x = 'subjID', by.y = 'Subject')
...
Instead of use the exploratory correlation analysis, we used a more principled way to explore the correlation between behavioral data (RT, dprime, etc) and scale data. More specifically, we used structural equation model.

First, we tested whether the behavioral data can form different latent variables.

```{r SEM2, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
behv.m1 <- 'behv =~ RT_Bad + 
RT_Good +
RT_Neutral +
RT_SD_Bad +
...
fit.behv.m1 <- lavaan::cfa(behv.m1, data = df.meta_1_wide, estimator = "MLR")
fit.behv.m2 <- lavaan::cfa(behv.m2, data = df.meta_1_wide, estimator = "MLR")
fit.behv.m3 <- lavaan::cfa(behv.m3, data = df.meta_1_wide, estimator = "MLR")
fit.behv.m4 <- lavaan::cfa(behv.m4, data = df.meta_1_wide, estimator = "MLR")

#Model Outputs
summary(fit.behv.m1, standardize = TRUE, fit.measures=TRUE)
summary(fit.behv.m2, standardize = TRUE, fit.measures=TRUE)
summary(fit.behv.m3, standardize = TRUE, fit.measures=TRUE)
summary(fit.behv.m4, standardize = TRUE, fit.measures=TRUE)
mrlID.comp <- lavaan::anova(fit.behv.m4, fit.behv.m2)
mrlID.comp

# plot the path
semPlot::semPaths(fit.behv.m1, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
semPlot::semPaths(fit.behv.m2, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
semPlot::semPaths(fit.behv.m3, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
semPlot::semPaths(fit.behv.m4, "std", edge.label.cex = 0.5, curvePivot = TRUE, intercepts = FALSE)
```
