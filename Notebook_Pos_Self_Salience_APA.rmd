...
To test the hypothesis that the valence effect of morality was driven by self-positivity, we conducted a representional similarity analysis on the behavioral data. which can (Kiani et al., 2007; Op de Beeck et al, 2001). The dissimilarity values could come from reaction times, accuracy, and the derived the indcies.  

More specifically, we split experiments into two categories: explicitly included self-relevance or not. Experiments that didn't explicitly included self-relevance are: exp 1a ~ 1c, exp 2, 5, 6a; experiment that included self-relevance: exp 3a, 3b, 6b, 7a, 7b. For experiments that didn't include self-relevance, we will calculate the corrleation matrix of mean RT, sd of RT, d prime, and four parameters derived from HDDM across all three valence levels, which will result a 21 by 21 correlation matrix. For the experiment included the self-relevance, we do the same analysis for self condition and other conditions separately, result in one 21 by 21 matrix for self and 21 by 21 matrix for other condition. Then, we will compare the representational matrix of morality (without self-relevence) to the matrix of morality of the self and of the other.
