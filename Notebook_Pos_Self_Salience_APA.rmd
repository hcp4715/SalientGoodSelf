...
### Valence effect
We synthesized effect size of *d* prime and RT from experiment 1a, 1b, 1c, 2, 5 and 6a for the valence effect. We reported the synthesized the effect across all experiments that tested the valence effect, using the mini meta-analysis approach. 
...
In this part, we combined the experiments that explicitly manipulated the self-reference and valence, which includes 3a, 3b, 6b, 7a, and 7b. For the positive versus negative contrast, data were from five experiments with 178 participants; for positive versus neutral and neutral versus negative contrasts, data were from three experiments with 108 participants.
...
As the reliability of the questionnaire can be found in [@Liu_2020_JOPD]. Then we calculated the correlation between the data from behavioral task and the questionnaire data. 
...
plot(df.meta_all_wide$RT_Good)
tmp1 <- scale(df.meta_all_wide$RT_Good)
plot(tmp1)

tmp <- df.meta_all_wide %>%
  dplyr::group_by(ExpID, Site) %>%
  dplyr::mutate(RT_Good_norm = scale(RT_Good))
  scale(df.q_scores.v$SlfEst)
plot(tmp$RT_Good_norm)
...
res.cor_bayes <- df.corr_norm %>%
  dplyr::select(-c(ExpID:Sex)) %>%
  correlation::correlation(., bayesian = T, bayesian_test = 'rope')
...
df.corr_norm %>% 
  dplyr::select(mrlslfImg, dprime_Good) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

df.corr_norm %>% 
  dplyr::select(mrlslfImg, Good_v) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

df.corr_norm %>% 
  dplyr::select(SlfEst, RT_SD_Neutral) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

ggplot(data = df.corr, aes(x = SlfEst, y = RT_SD_Neutral)) + 
  geom_point() +
  geom_smooth(method=lm)

df.corr_norm %>% 
  dplyr::select(SlfEst, dprime_Bad) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

ggplot(data = df.corr, aes(y = SlfEst, x = dprime_Bad)) + 
  geom_point() +
  geom_smooth(method=lm)

df.corr_norm %>% 
  dplyr::select(SelfBad, RT_Bad) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

df.corr_norm %>% 
  dplyr::select(mrlIdExt, Good_a) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)

df.corr_norm %>% 
  dplyr::select(mrlIdExt, Neutral_v) %>%
  tidyr::drop_na() %>%
  bootES::bootES(., effect.type = 'r', plot = T)
...
ggplot(data = df.corr_norm, aes(x = Good_v, y = mrlslfImg)) + 
  geom_point() + 
  geom_smooth(method=lm)

ggplot(data = df.corr, aes(x = Good_a, y = GoodNeut)) + 
  geom_point() +
  geom_smooth(method=lm)

ggplot(data = df.corr, aes(x = mrlslfImg, y = Good_v)) + 
  geom_point() +
  geom_smooth(method=lm)
