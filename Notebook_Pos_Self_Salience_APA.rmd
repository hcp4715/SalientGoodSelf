...
effectList <- c('Good-Bad','Good-Neut','Neut-Bad',
                'Good-Bad_S','Good-Neut_S','Neut-Bad_S',
                'Good-Bad_O','Good-Neut_O','Neut-Bad_O')

for (expName in unique(df.meta_d$ExpID)){
  for (effectName in effectList)){
    tmpdata <- df.meta_d %>% dplyr::filter(ExpID == expName)
    if (effectName == 'Good_Bad'){
      ...
      dataCond2 <- tmpdata %>% dplyr::filter(Valence == "Bad")
    }else if (effectName == 'Neut_Bad_S' & sum(!is.na(tmpdata$Identity))){
      next
    }
