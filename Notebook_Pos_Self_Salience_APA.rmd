...
df.ES_d <- data.frame(matrix(, nrow=length(unique(df.meta_d$ExpID))*length(effectList)*2, ncol=0)) %>%
...
for (DVName in c('RT','dprime')){
  if (DVName == 'RT'){
    metaData <- df.meta_rt %>% dplyr::rename(Value = RT)
  } else{
  ...
  for (expName in unique(metaData$ExpID)){
    for (effectName in effectList){
      ...
      M1  <- mean(dataCond1$Value) -> df.ES_d$M1[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName]
      SD1 <- sd(dataCond1$Value)   -> df.ES_d$SD1[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName] 
      M2  <- mean(dataCond2$Value) -> df.ES_d$M2[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName]
      SD2 <- sd(dataCond2$Value)   -> df.ES_d$SD2[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName]
      N   <- length(unique(dataCond2$Subject)) -> df.ES_d$N[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName]
      r   <- cor(dataCond1$Value,dataCond2$Value) -> df.ES_d$r[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName] 
      tmp2 <- d.sgpp(m.1 = M1, m.2 = M2, sd.1 = SD1, sd.2 = SD2,n = N, r = r)
      df.ES_d$ES[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName] <- tmp2[1,1]
      df.ES_d$ES.var[df.ES_d$DVtype == DVName & df.ES_d$ExpID == expName & df.ES_d$Effect == effectName] <- tmp2[1,2]
    }
  }
}

df.d_1_goodbad <- df.ES_d %>%
  dplyr::filter(DVtype == 'dprime' & Effect == 'Good_Bad')

M_D_GB <- metafor::rma(yi = df.d_1_goodbad$ES,
                       vi = df.d_1_goodbad$ES.var,
                       slab = df.d_1_goodbad$ExpID)
metafor::forest(M_D_GB)
...
Before analyzing, we first calculated the effect size based on the primary data of each experiment. Then, we used the '' function of the metafor to estimate the overall effect size.

To test different effect, conducted following analysis. First, we tested the effects of moral valence, regardless of self-relevance. To this end, we estimated the different between good vs. bad (13 experiments), good vs. neutral (11 experiments), and neutral vs. bad (11 experiments), for both *d* prime and RTs. Second, we examined the interaction between moral valence and identity. That is, we estimated the effect of valence for both self- and other- relevance conditions. Additionally, we examined the main effect of self-relevance, regardless of valence. To this end we estimated the differences between self vs. other conditions, regardless of the valence ().
