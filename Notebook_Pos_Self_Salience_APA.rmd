...
abstract: |
  Perception is not just a passive process but actively constructing. There are rich literature on different factors that impact the processing efficiency of our perceptual system. Two prominent examples are the self-prioritization and emotional attnetion. However, how these two factors interact in perceptual processes in largely unknown. We set out to examine the relationship between self-relevance and valence using a social associative paradigm (Sui et al., 2012). Through 11 experiments, we found that there was an robust good-self prioritization effect, which drives the valence effect when the self was not explicitly referred. These results was consistent with previous studies that good-self is core representation of the self and people's motivation to maitain a positive-self may influence perceptual process.These results suggested a positive self-bias in perception. That is, peopel literally "see" the positive side of themselves better.
...
In most of these experiments, participant were also asked to fill a battery of questionnaire after they finish the cognitive tasks. More specificially, ....... All the questionnaire data are open (see, Liu et al., 2019)
...
df1c <- read.csv(".\\exp1c\\rawdata_behav_exp1c_2014.csv", header = TRUE,
                   sep = ",", stringsAsFactors=FALSE, na.strings=c("","NA")) %>%
  dplyr::select(ï..Subject,Age, Handedness,Sex,TrialList1.Sample,BlockList.Sample,
...
df3a <- read.csv(".\\exp3a\\rawdata_behav_exp3a_2014.csv", header = TRUE,
  ...
  dplyr::mutate(Valence = derivedFactor("Neutral" = (Shape == "Normalself" | Shape == 'Normalother'), 
                ...
                #ExpID = "Exp3a",
                #Domain = "Morality",
                Subject = Subject + 3100) #%>%                                        # re-code the subject id
  #dplyr::select(CommonColnames)
...
df3a_dprime_s_anova_apa <- df3a_dprime_s_anova$aov %>% papaja::apa_print()
...
df3a_dprime_o_anova <- df3a.v.dprime_l %>%
  ...
  afex::aov_ez('Subject','dprime', ., within = c('Valence'))
df3a_dprime_o_anova_apa <- df3a_dprime_o_anova$aov %>% papaja::apa_print()
...
```
The results of reaction times of matchness trials showed similiar pattern as the *d* prime data. We found three-way intercation between matchness, and valence (`r df3b_RT_anova_apa$full$Matchness_Valence`) and then analyzed the matched trials and mismatched trials separately, as in previous experiments.
...
df3b_RT_anova_nm_s <- df3b.v.rt_m %>%
  ...
  afex::aov_ez('Subject','RT_m', ., within = c('Valence'))
df3b_RT_anova_nm_s_apa <- df3b_RT_anova_nm_s %>% papaja::apa_print()

df3b_RT_anova_nm_o <- df3b.v.rt_m %>%
  dplyr::filter(Matchness == "Mismatch" & Identity == "Other") %>%
  afex::aov_ez('Subject','RT_m', ., within = c('Valence'))
df3b_RT_anova_nm_o_apa <- df3b_RT_anova_nm_o %>% papaja::apa_print()
...
df4a_dprime_s_anova <- df4a.v.dprime_l %>%
  dplyr::filter(Identity == "Self")%>%
  afex::aov_ez('Subject','dprime', ., within = c('Valence'))
df4a_dprime_s_anova_apa <- df4a_dprime_s_anova$aov %>% papaja::apa_print()
...
df4a_dprime_o_anova <- df4a.v.dprime_l %>%
  dplyr::filter(Identity == "Other")%>%
  afex::aov_ez('Subject','dprime', ., within = c('Valence'))
df4a_dprime_o_anova_apa <- df4a_dprime_o_anova$aov %>% papaja::apa_print()
...
df5_RT_anova_apa <- df5_RT_anova %>% papaja::apa_print()

df5.v.rt_anova_m <- df5.v.rt_m %>%
  dplyr::filter(Matchness == "Match") %>%
  afex::aov_ez('Subject','RT_m', ., within = c('taskType','Valence'))
df5.v.rt_anova_m_apa <- df5.v.rt_anova_m %>% papaja::apa_print()
...
To get a better estimation of the effect in the current study, we combined the data of the 11 experiments described above and 2 experiments from another study (Hu, Lan, Macrae, & Sui, 2019). More specifically, we conducted random effect model meta-analyses of the effect size of *d*’ and RTs across our 13 experiments. Mini meta-analysis has been shown to be an effective way to get more precise and stable estimation of effect (Goh, Hall, & Rosenthal, 2016). We conducted a mini meta-analysis to estimate the effects observed in current study. 

## Methods
### Software
Mini Meta-analysis was carried out in R 3.6. As for the meta-analysis of the effect size of d’ and RTs, we used “metafor” package (Viechtbauer, 2010). 

### Data analysis
We first calculated the mean of *d* prime and RT of each condition for each participant, then calculate the effect size (Cohen's d) and variance of the effect size for each contrast (i.e., Good v. Bad, Good v. Neutral, and Neutral v. Bad). Cohen'd and its variance were estimated using the following formula (Cooper, Hedges, & Valentine, 2009):

$$Cohen's d = \frac {(M_{1} - M_{2})}{\sqrt {(sd_{1}^2 + sd_{2}^2) - 2*r*sd_{1}*sd_{2}}} * \sqrt {2*(1-r)}$$
...
To avoid stituation that some participant participated more than one experiment, we inspected the all the available participant information and markered those participants and only include their results from the first participation. Also, 24 participants were intentionally recruited to participate both exp 1a and exp 2, only results from exp 1a were included in the current meta-analysis.


```{r prep exp7,echo=FALSE,results='hide'}
...
```{r combine valid Data,echo=FALSE,results='hide'}
...
df3a.meta.d <- df3a.meta.d %>% dplyr::filter(!Subject %in% c(3113, 3112, 3143, 3146)) # exclude participants from ex1b, 1c, and 2
df3a.meta.rt <- df3a.meta.rt %>% dplyr::filter(!Subject %in% c(3113, 3112, 3143, 3146)) # exclude participants from ex1b, 1c, and 2
...
df.res.meta.all <- data.frame(matrix(, nrow=3*3*2, ncol=0)) %>%
  dplyr::mutate(DVtype = rep(c('RT', 'dprime'), each = 3*3),
...
for (DVName in c('RT','dprime')){
  for (effectName in effectList){
    ...
    #p_meta <- metafor::forest(tmp.meta.res)
    #if (is_empty(p_meta_list)){
    #  p_meta_list[1] = p_meta
    #} else{
    #  p_meta_list = c(p_meta_list, p_meta)
    #}
    #magicfor::put(p_meta)
  }
}
...
df.res.meta.all$EffectType <- rep(c('Good_Bad','Good_Neut','Neut_Bad'), 3*2)

df.res.meta.dprime <- df.res.meta.all %>% dplyr::filter(DVtype == 'dprime') %>%
...
df.res.meta.rt <- df.res.meta.all %>% dplyr::filter(DVtype == 'RT') %>%
  dplyr::mutate(Group = factor(Group, levels = c("Overall", "Self", "Other")),
                EffectType = factor(EffectType, levels = c('Good_Bad','Good_Neut', 'Neut_Bad')))

p_meta_dprime <- ggplot(df.res.meta.dprime, aes(x=Group, y=Cohen_d, fill=EffectType)) + 
  geom_bar(stat="identity", color=NA, 
           position=position_dodge()) +
  geom_errorbar(aes(ymin=Cohen_d-se, ymax=Cohen_d+se), width=.2,
...
p_meta_rt <- ggplot(df.res.meta.rt, aes(x=Group, y=Cohen_d, fill=EffectType)) + 
  geom_bar(stat="identity", color=NA, 
           position=position_dodge()) +
  geom_errorbar(aes(ymin=Cohen_d-se, ymax=Cohen_d+se), width=.2,
                 position=position_dodge(.9)) +
  coord_cartesian(ylim=c(-0.9, 0.2))+
  scale_y_reverse()+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
  ylab("Cohen' d of  RTs")+
...
In total, we calculated nine meta-analyses for both reaction times and *d* prime. First, we estimated the effects of valence regardless of self-referential (Good-Bad, Good-Neutral, and Neutral-Bad), The Good-Bad contrast included 13 experiments (1a - 7b, N = `r df.ES_sum$Nsubj[df.ES_sum$Effect == 'Good_Bad' & df.ES_sum$DVtype == 'dprime']`) while the Good-Neutral and Neutral-Bad contrasts included 11 experiments (1a ~ 6b, N = `r df.ES_sum$Nsubj[df.ES_sum$Effect == 'Neut_Bad' & df.ES_sum$DVtype == 'dprime']`). Then we combined the experiments with the variable of self-referential, and calculated the effect of valence for self-referential and other-referential separately. For the Good-Bad contrast, both self- and other-referential condition included 7 experiments (3a, 3b, 4a, 4b, 6b, 7a, 7b, N = `r df.ES_sum$Nsubj[df.ES_sum$Effect == 'Good_Bad_S' & df.ES_sum$DVtype == 'dprime']`), while for the Good-Neutral and Neutroal contrast, both conditions included 5 experiments (3a, 3b, 4a, 4b, 6b, N = `r df.ES_sum$Nsubj[df.ES_sum$Effect == 'Neut_Bad_S' & df.ES_sum$DVtype == 'dprime']`).


## Results

```{r 'meta-all-dprime-rt', fig.cap="Meta-analysis of RT and *d* prime of all experiments.", fig.height=6, fig.width=15, fig.align="center", warning=FALSE}

multiplot(p_meta_rt, p_meta_dprime, cols = 2)

```


Figure \@ref(fig:meta-all-dprime-rt) shows meta-analytic results for the effect of *d* prime and reaction times from Good-Bad, Good-Neutral, and Neutral-Bad contrast. 

Across all experiments, we found that the good-association condition has advantage over bad conditions for both RT (Cohen's d = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']`]) and *d* prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Bad']`]). Also the good-association has advantages over the neutral condition for both RT (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']`]) and *d* prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Good_Neut']`]). But the neutral condition did not differ from the bad conditions for d prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']`]) but slightly faster on RT, RT Cohen's (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Overall' & df.res.meta.all$EffectType == 'Neut_Bad']`]).

When we distinguish between self-referential and other-referential conditions, it is clear that the over all effect was mainly stem from the self-referential conditions: The good-association condition has advantage over bad conditions for both RT (Cohen's d = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']`]) and *d* prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Bad']`]), and over neutral condition for both  both RT (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']`]) and *d* prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Good_Neut']`]), but not for the *d* prime between neutral and bad on RT (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']`]) or *d* prime (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Self' & df.res.meta.all$EffectType == 'Neut_Bad']`]). 

For the other condition, no differences were observed for *d* prime: Good vs. Bad  (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']`]); good vs. neutral (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']`]); neutral vs. bad (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'dprime' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']`]). But the effect on RT has the similar pattern as the overall effect, with much small effect size on Good vs. Bad, (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Bad']`]) and Good vs. Neutral, (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Good_Neut']`]), and similar effect size on neutral vs. bad condition,  (Cohen's *d* = `r df.res.meta.all$Cohen_d[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']`, 95%CI[`r df.res.meta.all$CI_low[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']` `r df.res.meta.all$CI_upp[df.res.meta.all$DVtype == 'RT' & df.res.meta.all$Group == 'Other' & df.res.meta.all$EffectType == 'Neut_Bad']`]).



# Behavioral-Traits correlation analysis
