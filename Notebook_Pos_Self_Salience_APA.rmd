...
## Method
```{r loadingData_5a,echo=FALSE,results='hide'}
df5a <- read.csv(".\\exp5a_erp1\\rawdata_erp_exp5a_2014.csv",header = TRUE, sep = ",",
...
# number of participant who practiced but not in the formal experiment
#nQuit <- length(unique(df5a$Subject[is.na(df5a$BlockNo)])) - length(unique(df5a$Subject[!is.na(df5a$BlockNo)]))
...
# The rate of excluded trials in valid data
df5a.invalid_trial_rate   <- df5a %>%
  dplyr::filter(!is.na(BlockNo)) %>%
  dplyr::filter(!(Subject %in% df5a.excld.sub$Subject)) %>%   # exclude the invalid subjects
  dplyr::summarize(rate = length(RT[RT <= 200 & ACC == 1])/length(RT)) %>%
  dplyr::pull()
...
### Participants
`r df5a.T.basic$N` college students (`r df5a.T.basic$Nf` female, age = `r df5a.T.basic$Age_mean` $\pm$ `r df5a.T.basic$Age_sd`) participated the current study, all of them were from Tsinghua Universtiy in 2014. Informed consent was obtained from all participants prior to the experiment according to procedures approved by a local ethics committee. No participant was excluded from behavioral analysis.
...
### *d* prime
```{r analyzing for d prime_e5a, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# calculating the dprime 
df5a.v.dprime_l <- df5a.v %>%
  dplyr::mutate(sdt = mosaic::derivedFactor("hit" = (ACC ==1 & Matchness == "Match"), # code as hit
                                    "CR" = (ACC ==1  & Matchness == "Mismatch"),      # correct reject
                                    "miss" = (ACC == 0 & Matchness == "Match"),       # miss
                                    "FA" = (ACC == 0 & Matchness == "Mismatch"),      # false alarm
...
df5a_RT_anova <- afex::aov_ez('Subject','RT_m',df5a.v.rt_m,     # using afex's function 
                                  within = c('Matchness','Valence'))
df5a_RT_anova_apa <- df5a_RT_anova %>% papaja::apa_print()
```


```{r 5a_RT_match, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# match trials
df5a.v.rt_m1 <- df5a.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match") %>%
  dplyr::group_by(Site,Subject,Matchness, Valence) %>%
...
df5a_RT_anova_m <- afex::aov_ez('Subject','RT_m',df5a.v.rt_m1,     # using afex's function 
                                  within = c('Valence'))
df5a_RT_anova_m_apa <- df5a_RT_anova_m %>% papaja::apa_print()

posthoc_5a_rt <- emmeans::emmeans(df5a_RT_anova_m, "Valence") # compare each valence for both self and other condition
# pairs(posthoc_5a_rt)
...
df5a_RT_anova_nm <- afex::aov_ez('Subject','RT_m', df5a.v.rt_m2,     # using afex's function 
                                  within = c('Valence'))
df5a_RT_anova_nm_apa <- df5a_RT_anova_nm %>% papaja::apa_print()
```
We found intercation between Matchness and Valence (`r df5a_RT_anova_apa$full$Matchness_Valence`) and then analyzed the matched trials and mismatched trials separately, as in experiment 2. For matched trials, we found the effect of valence `r df5a_RT_anova_m_apa$full$Valence`. For non-matched trials, there was no significant effect of Valence (`r df5a_RT_anova_nm_apa$full$Valence`). Post-hoc *t*-tests revealed that shapes associated with Good Person (mean = 550, SE = 13.8) were responded faster than Neutral-Person (501, SE = 14.7), (*t*(24) = -5.171, *p* = 0.0001) and Bad Person (523, SE = 16.3), *t*(24) = -8.137, *p* < 0.0001)., and Neutral is faster than Bad-Person condition (*t*(32) = -3.282, *p* = 0.0085).


# Experiment 5b: EEG study 2
Experiment 5b was conducted to study the neural correlates of the prioritization effect of positive self, i.e., the neural underlying of the behavioral effect found int experiment 3a. However, as in experiment 5a, the procedure of this experiment was modified to adopted to ERP experiment. 

## Method
```{r loadingData_5b,echo=FALSE,results='hide'}
df5b_d1 <- read.csv(".\\exp5b_erp2\\rawdata_erp_exp5b_d1_2016.csv",header = TRUE, sep = ",",
                 stringsAsFactors=FALSE,na.strings=c("","NA")) %>%
  dplyr::filter(!is.na(BlockList.Sample)) %>%                                                   # select only form exp
  dplyr::rename(Subject = ï..Subject, 
                BlockNo = BlockList.Sample,
...
df5b_d2 <- read.csv(".\\exp5b_erp2\\rawdata_erp_exp5b_d2_2016.csv",header = TRUE, sep = ",",
                 stringsAsFactors=FALSE,na.strings=c("","NA")) %>%
  dplyr::filter(!is.na(BlockList.Sample)) %>%                                                   # select only form exp
  dplyr::rename(Subject = ï..Subject, 
                BlockNo = BlockList.Sample,
...
# The rate of excluded trials in valid data
df5b_d1.invalid_trial_rate   <- df5b_d1 %>%
  dplyr::filter(!is.na(BlockNo)) %>%
  dplyr::filter(!(Subject %in% df5b_d1.excld.sub$Subject)) %>%   # exclude the invalid subjects
  dplyr::summarize(rate = length(RT[RT <= 200 & ACC == 1])/length(RT)) %>%
  dplyr::pull()
...
df5b_d2.v   <- df5b_d2 %>%
  dplyr::filter(!is.na(BlockNo)) %>%
  dplyr::filter(!(Subject %in% df5b_d2.excld.sub$Subject)) %>%   # exclude the invalid subjects
  dplyr::filter(!(RT <= 200 & ACC == 1))                      # exclude < 200 trials

df5b_d2.v.basic     <- df5b_d2.v %>%
  dplyr::select(Site, Subject, Age, Sex) %>%
  dplyr::distinct(Subject, .keep_all = TRUE) %>%
  dplyr::summarise(N = length(Subject),
                   N_thu = length(Site[Site == "THU"]),
...
### Participants
`r df5b_d1.T.basic$N` college students (`r df5b_d1.T.basic$Nf` female, age = `r df5b_d1.T.basic$Age_mean` $\pm$ `r df5b_d1.T.basic$Age_sd`) participated the current study, all of them were recruited from Tsinghua Universtiy in 2016. Informed consent was obtained from all participants prior to the experiment according to procedures approved by a local ethics committee. For day 1's data, `r nrow(df5b_d1.excld.sub)` participant was excluded from the current analysis because of lower than 60% overall accuracy.`r df5b_d1.v.basic$N` college students (`r df5b_d1.v.basic$Nf` female, age = `r df5b_d1.v.basic$Age_mean` $\pm$ `r df5b_d1.v.basic$Age_sd`). For day 2's data, one participant dropp out , and leaving 22 participants  (`r df5b_d2.v.basic$Nf` female, age = `r df5b_d2.v.basic$Age_mean` $\pm$ `r df5b_d2.v.basic$Age_sd`) in day 2, all of them has overall accuracy higher than 60%.
...
## Results
Only the behavioral results were reported here.
```{r 5b_dprime_d1, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# calculate d prime
...
# anova for d prime
df5b_d1_dprime_anova <- afex::aov_ez('Subject','dprime',df5b_d1.v.dprime_l,  # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d1_dprime_anova_apa <- df5b_d1_dprime_anova$aov %>% papaja::apa_print()

# anova for self condition
df5b_d1_dprime_s_anova <- df5b_d1.v.dprime_l %>%
  dplyr::filter(Identity == "Self") %>%
  afex::aov_ez('Subject','dprime',., within = c('Valence'))

df5b_d1_dprime_s_anova_apa <- df5b_d1_dprime_s_anova$aov %>% papaja::apa_print()

posthoc_5b_d1_s_d <- emmeans::emmeans(df5b_d1_dprime_s_anova, "Valence") # compare each valence for both self and other condition
#pairs(posthoc_5b_d1_s_d)

# anova for Other condition
df5b_d1_dprime_o_anova <- df5b_d1.v.dprime_l %>%
  dplyr::filter(Identity == "Other") %>%
  afex::aov_ez('Subject','dprime',., within = c('Valence'))

df5b_d1_dprime_o_anova_apa <- df5b_d1_dprime_o_anova$aov %>% papaja::apa_print()
```
### Day one

#### *d* prime
There was evidence for the interaction between identity and valence, `r df5b_d1_dprime_anova_apa$full$Identity_Valence`. We further split the self- and other-relevant trials. For the self trials, there was significant effect of valence, `r df5b_d1_dprime_s_anova_apa$full$Valence`. Post-hoc comparison showed that the good-self condition (2.71, SE = 0.199) is better than both neutral-self (1.98, SE = 0.151), *t*(21) = 5.984, *p* < 0.001, and bad-self condition (2.07, SE = 0.154), *t*(21) = 6.555, *p* < 0.001. But there was no significant difference between bad-self and neutral-self, *t*(21) = -1.059, *p* = 0.549. For other trials, there was no significant effect of valuence, `r df5b_d1_dprime_o_anova_apa$full$Valence`.

```{r 5b_RT, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# anova for RT with 2*2*3 design
df5b_d1.v.rt_m <- df5b_d1.v %>%
  dplyr::filter(ACC == 1) %>%
  dplyr::group_by(Site,Subject,Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d1_RT_anova <- afex::aov_ez('Subject','RT_m',df5b_d1.v.rt_m,     # using afex's function 
                                  within = c('Matchness','Identity','Valence'))
df5b_d1_RT_anova_apa <- df5b_d1_RT_anova %>% papaja::apa_print()

df5b_d2.v.rt_m <- df5b_d2.v %>%
  dplyr::filter(ACC == 1) %>%
  dplyr::group_by(Site,Subject,Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d2_RT_anova <- afex::aov_ez('Subject','RT_m',df5b_d2.v.rt_m,     # using afex's function 
                                  within = c('Matchness','Identity','Valence'))
df5b_d2_RT_anova_apa <- df5b_d2_RT_anova %>% papaja::apa_print()
```

```{r 5b_d1_RT_match, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# Day 1: match trials
df5b_d1.v.rt_m1 <- df5b_d1.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d1_RT_anova_m <- afex::aov_ez('Subject','RT_m',df5b_d1.v.rt_m1,     # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d1_RT_anova_m_apa <- df5b_d1_RT_anova_m %>% papaja::apa_print()

### For matched, self trials
df5b_d1.v.rt_m1_s <- df5b_d1.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match" & Identity == "Self") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d1_RT_anova_m_s <- afex::aov_ez('Subject','RT_m',df5b_d1.v.rt_m1_s,     # using afex's function 
                                  within = c('Valence'))
df5b_d1_RT_anova_m_s_apa <- df5b_d1_RT_anova_m_s %>% papaja::apa_print()

posthoc_5b_d1_rt_s <- emmeans::emmeans(df5b_d1_RT_anova_m_s, 'Valence') # compare each valence for both self and other condition
# pairs(posthoc_5b_d1_rt_s)

### For matched, other trials
df5b_d1.v.rt_m1_o <- df5b_d1.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match" & Identity == "Other") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d1_RT_anova_m_o <- afex::aov_ez('Subject','RT_m',df5b_d1.v.rt_m1_o,     # using afex's function 
                                  within = c('Valence'))
df5b_d1_RT_anova_m_o_apa <- df5b_d1_RT_anova_m_o %>% papaja::apa_print()


# Day1: Mismatch trials
df5b_d1.v.rt_m2 <- df5b_d1.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Mismatch") %>%
  dplyr::group_by(Site,Subject,Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d1_RT_anova_nm <- afex::aov_ez('Subject','RT_m',df5b_d1.v.rt_m2,     # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d1_RT_anova_nm_apa <- df5b_d1_RT_anova_nm %>% papaja::apa_print()
```

#### RT
For the matched trials, there was interaction between identity and valence, `r df5b_d1_RT_anova_m_apa$full$Identity_Valence`. We split the self-relevant and other relevant trials separately. For the self condition, the valence effect is significant, `r df5b_d1_RT_anova_m_s_apa$full$Valence`. The Self-good (484, SE = 13.2) is faster than self-neutral (543, SE = 16.7) , *t* = -4.521, *p* = 0.0005, *df* = 21 and self-bad condition (535, SE = 18.4),  *t* = -4.489, *p* = 0.0006, *df* = 21. but not significant different between neutral and bad condition, *t* = 0.689, *p* = 0.772, *df* = 21. For other condition, there was no effect of valence, `r df5b_d1_RT_anova_m_o_apa$full$Valence`. 
...
df5b_d2_dprime_anova <- afex::aov_ez('Subject','dprime',df5b_d2.v.dprime_l,  # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d2_dprime_anova_apa <- df5b_d2_dprime_anova$aov %>% papaja::apa_print()
#posthoc_5b_d2_d <- emmeans::emmeans(df5b_d1_dprime_anova, "Valence") # compare each valence for both self and other condition
#pairs(posthoc_5b_d2_d)

# anova for self condition
df5b_d2_dprime_s_anova <- df5b_d2.v.dprime_l %>%
  dplyr::filter(Identity == "Self") %>%
  afex::aov_ez('Subject','dprime',., within = c('Valence'))

df5b_d2_dprime_s_anova_apa <- df5b_d2_dprime_s_anova$aov %>% papaja::apa_print()

posthoc_5b_d2_s_d <- emmeans::emmeans(df5b_d2_dprime_s_anova, "Valence") # compare each valence for both self and other condition
#pairs(posthoc_5b_d2_s_d)

# anova for Other condition
df5b_d2_dprime_o_anova <- df5b_d2.v.dprime_l %>%
  dplyr::filter(Identity == "Other") %>%
  afex::aov_ez('Subject','dprime',., within = c('Valence'))

df5b_d2_dprime_o_anova_apa <- df5b_d2_dprime_o_anova$aov %>% papaja::apa_print()

```
#### *d* prime
There was evidence for the interaction between identity and valence, `r df5b_d2_dprime_anova_apa$full$Identity_Valence`.  We further split the self- and other-relevant trials. For the self trials, there was significant effect of valence, `r df5b_d2_dprime_s_anova_apa$full$Valence`. Post-hoc comparison showed that the good-self condition (2.71, SE = 0.214) is better than both neutral-self (2.43, SE = 0.175), *t*(21) = 2.98, *p* = 0.0189, and bad-self condition (2.43, SE = 0.199), *t*(21) = 3.93, *p* = 0.0021. But there was no significant difference between bad-self and neutral-self, *t*(21) = -0.097, *p* = 0.995. For other trials, there was no significant effect of valuence, `r df5b_d2_dprime_o_anova_apa$full$Valence`.

```{r 5b_d2_RT_match, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# Day2: match trials
df5b_d2.v.rt_m1 <- df5b_d2.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d2_RT_anova_m <- afex::aov_ez('Subject','RT_m',df5b_d2.v.rt_m1,     # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d2_RT_anova_m_apa <- df5b_d2_RT_anova_m %>% papaja::apa_print()

posthoc_5b_d2_rt <- emmeans::emmeans(df5b_d2_RT_anova_m, c('Identity',"Valence")) # compare each valence for both self and other condition
# pairs(posthoc_5b_d2_rt)

df5b_d2.v.rt_m1_s <- df5b_d2.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match" & Identity == "Self") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d2_RT_anova_m_s <- afex::aov_ez('Subject','RT_m',df5b_d2.v.rt_m1_s,     # using afex's function 
                                  within = c('Valence'))
df5b_d2_RT_anova_m_s_apa <- df5b_d2_RT_anova_m_s %>% papaja::apa_print()

posthoc_5b_d2_rt_s <- emmeans::emmeans(df5b_d2_RT_anova_m_s, 'Valence') # compare each valence for both self and other condition
# pairs(posthoc_5b_d2_rt_s)

### For matched, other trials
df5b_d2.v.rt_m1_o <- df5b_d2.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Match" & Identity == "Other") %>%
  dplyr::group_by(Site, Subject, Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d2_RT_anova_m_o <- afex::aov_ez('Subject','RT_m',df5b_d2.v.rt_m1_o,     # using afex's function 
                                  within = c('Valence'))
df5b_d2_RT_anova_m_o_apa <- df5b_d2_RT_anova_m_o %>% papaja::apa_print()

# Day2: Mismatch trials
df5b_d2.v.rt_m2 <- df5b_d2.v %>%
  dplyr::filter(ACC == 1 & Matchness == "Mismatch") %>%
  dplyr::group_by(Site,Subject,Matchness, Identity, Valence) %>%
  dplyr::summarise(RT_m = mean(RT),
                   Ntrial = length(RT)) %>%
...
df5b_d2_RT_anova_nm <- afex::aov_ez('Subject','RT_m',df5b_d2.v.rt_m2,     # using afex's function 
                                  within = c('Identity','Valence'))
df5b_d2_RT_anova_nm_apa <- df5b_d2_RT_anova_nm %>% papaja::apa_print()
```

#### RT
For the matched trials, the interaction between identity and valence, `r df5b_d2_RT_anova_m_apa$full$Identity_Valence`. As in previous studies, we splited the self- and other-relevant trials. For the self condition, the valence effect is significant, `r df5b_d2_RT_anova_m_s_apa$full$Valence`. The Self-good (480, SE = 16.9) is faster than self-neutral (504, SE = 17.3) , *t* = -2.289, *p* = 0.0795, *df* = 21 and self-bad condition (508, SE = 17.9),  *t* = -4.342, *p* = 0.0008, *df* = 21. but not significant different between neutral and bad condition, *t* = -0.503, *p* = 0.871, *df* = 21. For other condition, there was no effect of valence, `r df5b_d2_RT_anova_m_o_apa$full$Valence`. 
