...
For the questinnaire part, we are most interested in the self-rated distance between different person and self-evaluation related questionnaires (e.g., moral-self identity, self-esteem etc.). Other questionnaires were not planned to correlated with behavioral data, but we explored the results that might be theoretically interest, e.g., self-esteem, moral identity.
...
RelSlfEstname <- c("Relat_SE_1","Relat_SE_2","Relat_SE_3","Relat_SE_4",
                   "Relat_SE_5","Relat_SE_6","Relat_SE_7","Relat_SE_8") 
# moral identity
...
df.q_scores.v <- df.scales.v %>%
  dplyr::mutate(SlfEst = rowMeans(.[, SlfEstNames],na.rm = F),
                RelSlfEst = rowMeans(.[, RelSlfEstname], na.rm = F),
                mrlId = rowMeans(.[, mrlIdNames], na.rm = F),
                mrlslfImg = rowMeans(.[, mrlslfImgNames], na.rm = F),
                BFI_A = rowMeans(.[, BFI_ANames], na.rm = F),
                BFI_C = rowMeans(.[, BFI_CNames], na.rm = F),
                BFI_N = rowMeans(.[, BFI_NNames], na.rm = F),
                BFI_O = rowMeans(.[, BFI_ONames], na.rm = F),
                BFI_E = rowMeans(.[, BFI_ENames], na.rm = F)) %>%
  dplyr::select(subjID, SlfEst, RelSlfEst, mrlId, mrlslfImg,
                BFI_A, BFI_C, BFI_N, BFI_O, BFI_E)
...
RelSlfEstAlpha <-  psych::alpha(df.scales.v[,RelSlfEstname], keys=RelSlfEstKeys)  # calculate the alpha coefficient of Relational Self-Esteem
print(RelSlfEstAlpha$total)  # print the alpha for Relational Self-Esteem
RelSlfEstOmega <- psych::omega(df.scales.v[,RelSlfEstname])  
