...
## data for data without reference
tmp1 <- df.meta_rt_1 %>%
...
#expNameList <- c("Exp1a", "Exp1b", "Exp1c", "Exp2", "Exp3a","Exp3b", "Exp4a", "Exp4b","Exp5","Exp6a","Exp6b","Exp7a","Exp7b")
...
# exp_ls_1 <- c('df1a', 'df1b', 'df1c', 'df2', 'df4b' , 'df5', 'df6a')
...
df1b.v.hddm_params <- read.csv(".\\HDDM\\df1b_hddm_params.csv",header = TRUE, sep = ",",
  ...
  dplyr::rename(param = v1)  %>% dplyr::filter(Matchness=='Match') %>% dplyr::select(- c(v2, Matchness)) %>%
  tidyr::pivot_wider(., names_from = c('Valence', 'param'), values_from = 'mean') %>%
  dplyr::mutate(ExpID = 'Exp1b')

df5.v.hddm_params <- read.csv(".\\HDDM\\df5_hddm_params.csv",header = TRUE, sep = ",",
                 stringsAsFactors=FALSE,na.strings=c("","NA")) %>%
  dplyr::rename(Subject = subj_idx, Matchness = match, Valence = val) %>%
  dplyr::select(Subject, Matchness, Valence, knode_name, mean) %>%
  tidyr::drop_na() %>%
...
df6a.v.hddm_params <- read.csv(".\\HDDM\\df6a_hddm_params.csv",header = TRUE, sep = ",",
                 stringsAsFactors=FALSE,na.strings=c("","NA")) %>%
  dplyr::rename(Subject = subj_idx, Matchness = match, Valence = val) %>%
  dplyr::select(Subject, Matchness, Valence, knode_name, mean) %>%
  tidyr::drop_na() %>%
...
df.meata_hddm_1 <- rbind(df1a.v.hddm_params, df1b.v.hddm_params, df5.v.hddm_params)

df.meta_1_wide <- merge(df.meta_1_wide,df.meata_hddm_1)

## Network analysis
# group the items for later legend
nodeNames <- colnames(df.meta_1_wide[,9:26])
gr <- list(c(1, 2, 3),
           c(4, 5, 6),
           c(7, 8, 9),
           c(10, 11, 12),
           c(13, 14, 15),
           c(16, 17, 18))
df.meta_1_corr <- qgraph::cor_auto(df.meta_1_wide[,9:26])
df.meta_1_nw <- bootnet::estimateNetwork(df.meta_1_corr, default = "EBICglasso")
...
network1_all_G <- plot(df.meta_1_nw,
