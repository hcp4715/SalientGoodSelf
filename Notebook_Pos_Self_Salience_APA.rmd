...
fun_sdt_val_id <- function(exp_name) {
  ...
  brms::brm(saymatch ~ 0 + Valence:Identity + Valence:Identity:ismatch + 
              (0 + Valence:Identity + Valence:Identity:ismatch | Subject),
            family = bernoulli(link="probit"),
            data = .,
            control = list(adapt_delta = .99),
            iter = 4000,
...
fun_rt_val_id <- function(exp_name) {
  ...
  m <- df %>%
    ...
    dplyr::mutate(ismatch = ifelse(Matchness == 'Match', 1, 0),
                  Valence = factor(Valence, levels = c('Neutral', 'Bad', 'Good')),
                  Identity = factor(Identity, levels=c('Self', 'Other'))) %>%
    brms::brm(RT_sec ~ ismatch*Valence*Identity + (ismatch*Valence*Identity | Subject),
              family = shifted_lognormal(),
              data = ., control = list(adapt_delta = .99),
              iter = 4000,
              thin = 2,
