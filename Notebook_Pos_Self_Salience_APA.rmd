...
```{r analyzing for d prime_4b, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
...
```{r analyzing for d prime_exp5, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
...
p_df_5_dprime <- df.ES_5 %>%
  dplyr::filter(DVtype == "dprime") %>%
  ggplot(., aes(x=Domain, y=ES, color=EffectType, fill=EffectType)) + 
  geom_pointrange(aes(ymin=ES - 1.96*sqrt(ES.var), ymax = ES + 1.96*sqrt(ES.var)), 
                  position = position_dodge(width = 0.4),
                  shape=18, size=1) +
  geom_hline(yintercept=0, size=1, color='black') +
  ggtitle('A: Valence effect on d prime') +
  coord_cartesian(ylim=c(-1.5, 1.5))+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
  ylab(expression(paste("Cohen's ",italic("d"), sep = ' ')))+
...
p_df_5_RT <- df.ES_5 %>%
  ...
  geom_pointrange(aes(ymin=ES - 1.96*sqrt(ES.var), ymax = ES + 1.96*sqrt(ES.var)), 
                  ...
                  shape=18, size=1) +
  geom_hline(yintercept=0, size=1, color='black') +
  ggtitle('B: Valence effect on RT') +
  coord_cartesian(ylim=c(-1.5, 1.5))+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
  ylab(expression(paste("Cohen's ",italic("d"), sep = ' ')))+
...
multiplot(p_df_5_dprime, p_df_5_RT, cols=2)
```
...
To qunatify the pattern similarity between moral valence without referential processing and with referential, we used multiple ways. We used the mean RT, SD of RT, *d* prime, and drift rate derived from HDDM across all three valence levels, which will result a 12 variables.We then used the following three apporachs to estimate the similarity between moral valence without any referential and moral valence with self-referential, as well as between valence without reference with valence with other-reference. First, we estimated the geomatric of three valence by mapping all participants's data into four-demension space: mean RT, SD of RT, *d* prime, and drift rate, and then calculate the Euclidean distance. Then we calculate the Euculid distand between these condition. Second, we applied a Psychometric Network model, in which we construct a network in which each node represents a variable and the edge between node represent their partial correlation. We used LASSO penality so that the final network would be sparse. Three network will be constructed: without reference, self-reference, and other reference. Third, we adopted the classic representative similarity analysis, i.e., we constructed three disimilarity matrices for without reference, self-reference, and other-reference. And then we compare those three disimilarity matrices. 

We also constructed a network based on the self-rated the psychological distance. (Impossible position of self).
...
nodeNames <- colnames(df.meta_1_wide[,15:23])
gr <- list(c(1, 2, 3),
           c(4, 5, 6),
           c(7, 8, 9),
           c(10, 11, 12),
           c(13, 14, 15),
           c(16, 17, 18))
#df.meta_1_corr <- qgraph::cor_auto(df.meta_1_wide[, 6:23])
df.meta_1_nw <- bootnet::estimateNetwork(df.meta_1_wide[, 15:23], default = "EBICglasso")

#type <- rep('g', 9)                 #g=gaussian, 20 = number of nodes in the network

#df.meta_1_fit  <- mgm::mgm(na.omit(df.meta_1_wide[,9:17]), type=type, lev=rep(1,20)) 
#df.meta_1_pred <- predict(df.meta_1_fit, na.omit(df.meta_1_wide[,9:17]))

### Plot networks
# network of all data
pdf("Network_1.pdf", width=10, height=7)
network_meta_1 <- plot(df.meta_1_nw,
...
## Network analysis
# group the items for later legend
nodeNames2 <- colnames(df.meta_2_wide[,16:24])
gr <- list(c(1, 2, 3),
           c(4, 5, 6),
           c(7, 8, 9),
           c(10, 11, 12),
           ...
           c(16, 17, 18))
#df.meta_1_corr <- qgraph::cor_auto(df.meta_1_wide[, 6:23])
df.meta_2_nw_self <- df.meta_2_wide %>%
  dplyr::filter(Identity == "Self") %>%
  dplyr::select(16:24) %>%
  tidyr::drop_na() %>%
  bootnet::estimateNetwork(., default = "EBICglasso")

df.meta_2_nw_oth <- df.meta_2_wide %>%
  dplyr::filter(Identity == "Other") %>%
  dplyr::select(16:24) %>%
  tidyr::drop_na() %>%
  bootnet::estimateNetwork(., default = "EBICglasso")
...
# network of all data
#pdf("Network_1.pdf", width=10, height=7)
network_meta_2_s <- plot(df.meta_2_nw_self,
...
network_meta_2_o <- plot(df.meta_2_nw_oth,
...
multiplot(network_meta_1,network_meta_2_s,network_meta_2_o)
```
...
#dataList <- list(df1a.v, df1b.v, df1c.v, df2.v, df3a.v, df3b.v, df df4a.v, df4b.v, df5.v, df6a.v, df6b_d1.v, df7a_m.v, df7b_m.V)
