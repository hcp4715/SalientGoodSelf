...
[Effect of good character] The robust effect of the prioritization of good character provide solid evidence for the notion that morality do have an impact in perceptual decision making [van Bavel]. Previous research reported the effect of moral perception [] but the mechanism is disputed. For example, [Anderson et al 2011] reported that faces associated with bad social behavior capture attention more rapidly, however, independent team failed to replicate the effect []. Another studies by [] found that moral words are more likely to be judged as words when it was presented sublimial [], however, this effect may caused by semantic priming instead of morality []. In the current study, we used associative learning task where neutral stimuli, geometric shapes, acquired a moral meaning in the lab and then perform the perceptual matching task, which eliminated the possibility of semantics priming [Firestone; Jussim et al., 2016] or other confounding factors. Also, the effect was replicated from all five different sample, suggestion a robust prioritization effect of good character.

Note that there is one critical difference between the current task and previous experiments that aimed at detection the effect of morality in lower-level information processing. Previous studies used the detection task, where participants need to detect whether or a stimulus exist. for example, in Anderson et al study, the task is to detect a face in awareness, in Gantman et al's study, the task is to detect existence of a word. By contrast, in our task, participants were required to recognize the stimuli pair and then make a decision. Previous review found that even for affective stimuli, results from detection and recognition tasks are not always consistent: while negativity effect are usual observed in the detection task, positiivity effect are often observed in recognition tasks []. This different may explains why in our experiments we found robust positivity effect for moral character. 
