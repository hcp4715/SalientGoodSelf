...
df5 <- read.csv(".\\exp5_specificity\\rawdata_behav_5_specificity_2016.csv",header = TRUE, sep = ",",stringsAsFactors=FALSE) %>%
  dplyr::rename(BlockListM = BlockListMoral) %>%
  dplyr::mutate(taskType = derivedFactor("Emotion"  = (Label == "sad" | Label == "happy" | Label == "neutral"),
                                         ...
                                         method ="first", .default = NA)) %>%
  tidyr::replace_na(list(PracListE="",    PracListM="",    PracListP="",    PracListS="",
                         BlockListE="",   BlockListM="",   BlockListP="",   BlockListS="",
  ...
  tidyr::unite("PracList", PracListE,PracListM,PracListP,PracListS, sep = "")  %>%
  tidyr::unite("BlockList", BlockListE,BlockListM,BlockListP,BlockListS, sep = "") %>%
  tidyr::unite("ACC", TargetE.ACC,  TargetM.ACC,  TargetP.ACC,  TargetS.ACC, sep = "") %>%
  tidyr::unite("RESP", TargetE.RESP,  TargetM.RESP,  TargetP.RESP,  TargetS.RESP, sep = "") %>%
  tidyr::unite("RT", TargetE.RT,  TargetM.RT,  TargetP.RT,  TargetS.RT, sep = "") %>%
  
  dplyr::mutate(taskType = as.character(taskType)) %>%
  dplyr::filter(is.na(taskType))

  transmute_all(funs(ifelse(. == 1, deparse(substitute(.)), NA))(PracListE:BlockListS))

  tidyr::gather(bList, value, PracListE:BlockListS) %>%
  dplyr::mutate(bList = ifelse(bList == "BlockListMoral","BlockListM", bList)) %>%
  tidyr::separate(bList,c("listType","taskType"), sep = "List")
  dplyr::rename(Morality = morality) %>%
  dplyr::mutate(Site = "THU")
...
###Stimuli
4 sets of shapes were included (three circle, three rectangle, three kind of triangle, and three kinds of  quadrangle), each set of shape were paired with one category of words, counter-balanced across subjects. Besides the behavioral experiments, participants also finished questionnaires XXXXXXXX.
