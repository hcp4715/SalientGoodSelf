...
df.perdist <- df.scales %>%
  ...
  dplyr::select(expID, subjID, maxDist,
                SelfGoodraw, SelfNeutraw, SelfBadraw,
                GoodNeutraw, GoodBadraw, NeutBadraw) %>%
  dplyr::mutate(SelfGood = SelfGoodraw/maxDist,
                SelfNeut = SelfNeutraw/maxDist,
                SelfBad = SelfBadraw/maxDist,
                # SelfStra = SelfStraraw/maxDist, 
                GoodNeut = GoodNeutraw/maxDist, 
                GoodBad = GoodBadraw/maxDist, 
                NeutBad = NeutBadraw/maxDist) %>%
  dplyr::select(subjID, SelfGood, SelfNeut, SelfBad,
                GoodNeut, GoodBad, NeutBad) %>% # note, SelfStra may has many zeros that should be NA
  tidyr::drop_na()
```
...
mean(df.perdist$GoodBad) - 2.5*sd(df.perdist$GoodBad)
