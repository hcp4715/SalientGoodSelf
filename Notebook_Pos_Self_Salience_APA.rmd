...
Perceptual decision-making is an important window for understanding the cognition (Shadlen & Kiani, 2013).

The role of perception in social cognition has been largely ignored, but accumulating evidence revealed that perception provides rich information about our social cognition (Stolier & Freeman, 2016; Xiao, Coppin, & Bavel, 2016).
...
heatmap(x = dist.cor$r, col = col, symm = TRUE)
  
  #correlation::correlation(., method="spearman", p_adjust = 'none')

# Suppose the self as the anchor, use the self-distance to calc the distance between good, bad, and neutral
df.plot2 <- df.perdist %>%
  dplyr::mutate(GoodNeut_Self = abs(SelfGood_pd - SelfNeut_pd),
                NeutBad_Self = abs(SelfBad_pd - SelfNeut_pd),
                GoodBad_Self = abs(SelfBad_pd - SelfGood_pd)) %>%
  dplyr::select(-c(SelfGood_pd, SelfNeut_pd, SelfBad_pd)) %>%
  dplyr::rename(GoodNeut_NonSelf = GoodNeut_pd,
                GoodBad_NonSelf = GoodBad_pd,
                NeutBad_NonSelf = NeutBad_pd)  %>%
  ...
  dplyr::mutate(Conds_j = jitter(Conds, amount=.03))   # add gitter to x
...
cor_pairs_all <- res.cor_all %>%
  ...
  dplyr::filter(Parameter2 %in% colnames(df_hddm_param_all[3:11])) %>%
  dplyr::filter(p <= 0.1) %>%
  dplyr::arrange(p)
