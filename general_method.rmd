...
For questionnaire data, we only reported the subjective distance between different persons or moral character in the supplementary results and did not analyze other questionnaire data, which are described in [@Liu_2020_JOPD].
...
in which `RT_sec` is the reaction times data with the second as a unit. The other variables has the same meaning as the response data. 
