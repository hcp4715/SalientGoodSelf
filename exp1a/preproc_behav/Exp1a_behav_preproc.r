...
df1a.acc  <-  ddply(df1a.V,.(Subject,Matchness, Morality), summarise,
                     N = length(ACC),
                     countN = sum(ACC),
                     ACC = sum(ACC)/length(ACC))
...
for (i in 1:nrow(df1a.V)){
        if (df1a.V$ACC[i] == 1 & df1a.V$Matchness[i] == "Match"){
                df1a.V$sdt[i] <- "hit"
        } else if (df1a.V$ACC[i] == 1 & df1a.V$Matchness[i] == "Mismatch"){
                df1a.V$sdt[i] <- "CR"
        } else if (df1a.V$ACC[i] == 0 & df1a.V$Matchness[i] == "Match"){
                df1a.V$sdt[i] <- "miss"
        } else if (df1a.V$ACC[i] == 0 & df1a.V$Matchness[i] == "Mismatch"){
                df1a.V$sdt[i] <- "FA"
        }
}
...
# order the columns
df1a.V.sum_w <- df1a.V.sum_w[,c("Subject", "Sex","Age", "ACC_Match_Moral", "ACC_Match_Neutral", "ACC_Match_Immoral", "ACC_Mismatch_Moral",
...
df1a.v.sum_rt_acc_l <- merge(df1a.acc,df1a.V.RT.subj,by = c("Subject","Matchness","Morality"))
df1a.v.sum_rt_acc_l <- df1a.v.sum_rt_acc_l[order(df1a.v.sum_rt_acc_l$Subject),]
df1a.v.sum_rt_acc_l <- df1a.v.sum_rt_acc_l[,c("Subject","Matchness","Morality","N.x","countN","ACC","RT")]
colnames(df1a.v.sum_rt_acc_l) <- c("Subject","Matchness","Morality","Ntrials","corrtrials","ACC","RT")
